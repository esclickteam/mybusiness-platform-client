...
const COUNTDOWN = {
  title: row("Countdown", "ספירה לאחור", "Cuenta atrás", "Contagem regressiva", "عدّ تنازلي"),
  description: row("A dynamic timer for promos, launches, and events — add it from the editor → Plugins.", "טיימר דינמי למבצעים, השקות ואירועים — הוסיפו לעמוד דרך העורך → תוספים.", "Temporizador para promos y eventos — añádelo desde el editor → Plugins.", "Timer para promoções e eventos — adicione no editor → Plugins.", "مؤقت للعروض والفعاليات — أضفه من المحرر → إضافات."),
  pluginActive: row("Plugin active on the site", "תוסף פעיל באתר", "Plugin activo en el sitio", "Plugin ativo no site", "الإضافة نشطة في الموقع"),
  styleSize: row("Style and size", "סגנון וגודל", "Estilo y tamaño", "Estilo e tamanho", "النمط والحجم"),
  pageDisplay: row("On-page display", "תצוגה בעמוד", "Visualización en la página", "Exibição na página", "العرض في الصفحة"),
  size: row("Size", "גודל", "Tamaño", "Tamanho", "الحجم"),
  preview: row("Preview", "תצוגה מקדימה", "Vista previa", "Pré-visualização", "معاينة"),
  contentTime: row("Content and time", "תוכן וזמן", "Contenido y tiempo", "Conteúdo e tempo", "المحتوى والوقت"),
  headline: row("Title", "כותרת", "Título", "Título", "العنوان"),
  defaultTitle: row("The offer ends in", "המבצע מסתיים בעוד", "La oferta termina en", "A oferta termina em", "ينتهي العرض خلال"),
  endDate: row("End date and time", "תאריך ושעת סיום", "Fecha y hora de fin", "Data e hora de término", "تاريخ ووقت الانتهاء"),
  expiredMessage: row("Message when the offer ends", "הודעה כשהמבצע נגמר", "Mensaje al terminar", "Mensagem ao terminar", "رسالة عند انتهاء العرض"),
  defaultExpired: row("The offer has ended", "המבצע הסתיים", "La oferta terminó", "A oferta terminou", "انتهى العرض"),
  reverseUnits: row("Reverse unit order (seconds → months)", "סדר יחידות הפוך (שניות ← חודשים)", "Orden inverso (segundos → meses)", "Ordem inversa (segundos → meses)", "ترتيب عكسي (ثوانٍ → أشهر)"),
  unitFormat: row("How time is shown", "אופן הצגת זמן", "Cómo se muestra el tiempo", "Como o tempo é mostrado", "كيف يظهر الوقت"),
  monthsAsDays: row("Convert months to days (30 days = 1 month)", "המר חודשים לימים (30 יום = חודש)", "Convertir meses a días (30 días = 1 mes)", "Converter meses em dias (30 dias = 1 mês)", "حوّل الأشهر إلى أيام (30 يوماً = شهر)"),
  weeksAsDays: row("Convert weeks to days (7 days = 1 week)", "המר שבועות לימים (7 יום = שבוע)", "Convertir semanas a días (7 días = 1 semana)", "Converter semanas em dias (7 dias = 1 semana)", "حوّل الأسابيع إلى أيام (7 أيام = أسبوع)"),
  ...
  convertBothHint: row("All remaining time is shown as days only (months and weeks are converted).", "כל הזמן מוצג כימים בלבד (חודשים ושבועות מומרים).", "Todo el tiempo se muestra en días (meses y semanas convertidos).", "Todo o tempo aparece em dias (meses e semanas convertidos).", "يظهر كل الوقت كأيام فقط (تُحوَّل الأشهر والأسابيع)."),
  convertMonthsHint: row("Months convert to days. You can keep weeks for detail or convert them too.", "חודשים מומרים לימים. אפשר להשאיר שבועות לפירוט או להמיר גם אותם.", "Los meses pasan a días. Puedes dejar semanas o convertirlas también.", "Meses viram dias. Você pode manter semanas ou convertê-las também.", "الأشهر تُحوَّل لأيام. يمكن الإبقاء على الأسابيع أو تحويلها أيضاً."),
  convertWeeksHint: row("Weeks convert to days. Months stay as a separate card.", "שבועות מומרים לימים. חודשים נשארים כרטיס נפרד.", "Las semanas pasan a días. Los meses siguen en una tarjeta aparte.", "Semanas viram dias. Meses ficam em um cartão separado.", "الأسابيع تُحوَّل لأيام. الأشهر تبقى بطاقة منفصلة."),
  daysOnlyHint: row("Remaining time is shown as total days, then hours/minutes/seconds.", "כל הזמן הנותר מוצג כסה״כ ימים, ואחריו שעות/דקות/שניות.", "El tiempo restante se muestra en días totales, luego horas/minutos/segundos.", "O tempo restante aparece em dias totais, depois horas/minutos/segundos.", "يظهر الوقت المتبقي كإجمالي أيام ثم ساعات/دقائق/ثوانٍ."),
  weeksOnlyHint: row("Time is shown as total weeks. You can also add leftover days in the current week.", "הזמן מוצג כסה״כ שבועות. אפשר להוסיף גם ימים לשארית בתוך השבוע הנוכחי.", "Se muestra en semanas totales. También puedes añadir los días restantes de la semana.", "Aparece em semanas totais. Também pode somar os dias restantes da semana.", "يظهر كإجمالي أسابيع. يمكن إضافة الأيام المتبقية في الأسبوع الحالي."),
  effects: row("Effects", "אפקטים", "Efectos", "Efeitos", "تأثيرات"),
  effectType: row("Effect type", "סוג אפקט", "Tipo de efecto", "Tipo de efeito", "نوع التأثير"),
  effectWhen: row("When to show", "מתי להציג", "Cuándo mostrar", "Quando mostrar", "متى يُعرض"),
  colors: row("Colors, font, and shadow", "צבעים, פונט וצל", "Colores, fuente y sombra", "Cores, fonte e sombra", "ألوان وخط وظل"),
  font: row("Font", "פונט", "Fuente", "Fonte", "الخط"),
  background: row("Background", "רקע", "Fondo", "Fundo", "الخلفية"),
  cardColor: row("Card color", "צבע כרטיסיות", "Color de tarjetas", "Cor dos cartões", "لون البطاقات"),
  numberColor: row("Number color", "צבע מספרים", "Color de números", "Cor dos números", "لون الأرقام"),
  labelColor: row("Label color", "צבע תוויות", "Color de etiquetas", "Cor dos rótulos", "لون التسميات"),
  accentColor: row("Accent color", "צבע הדגשה", "Color de acento", "Cor de destaque", "لون التمييز"),
  shadow: row("Accent shadow", "צל להדגשה", "Sombra de acento", "Sombra de destaque", "ظل التمييز"),
  shadowColor: row("Shadow color", "צבע צל", "Color de sombra", "Cor da sombra", "لون الظل"),
  editorTip: row("In the editor: Add → Plugins → Countdown. The block appears at a default size — drag to resize like an image. Fixed floating is live-site only.", "בעורך: הוספה → תוספים → ספירה לאחור. הרכיב בגודל ברירת מחדל — גררו ושנו גודל כמו תמונה. צף קבוע — רק לאתר החי.", "En el editor: Añadir → Plugins → Cuenta atrás. Arrastra para cambiar el tamaño. Flotante fijo solo en el sitio en vivo.", "No editor: Adicionar → Plugins → Contagem. Arraste para redimensionar. Flutuante fixo só no site ao vivo.", "في المحرر: إضافة → إضافات → عدّ تنازلي. اسحب لتغيير الحجم. العائم الثابت للموقع الحي فقط."),
  styleCards: row("Cards", "כרטיסיות", "Tarjetas", "Cartões", "بطاقات"),
  styleCardsHint: row("Numbers in cards with a shadow", "מספרים בכרטיסים עם צל", "Números en tarjetas con sombra", "Números em cartões com sombra", "أرقام في بطاقات مع ظل"),
  stylePulse: row("Pulse", "פועם", "Pulso", "Pulso", "نبض"),
  stylePulseHint: row("Large numbers with a pulse animation", "מספרים גדולים עם אנימציית פעימה", "Números grandes con pulso", "Números grandes com pulso", "أرقام كبيرة مع نبض"),
  styleGradient: row("Gradient", "גרדיאנט", "Degradado", "Gradiente", "تدرّج"),
  styleGradientHint: row("Colorful background with white numbers", "רקע צבעוני עם מספרים לבנים", "Fondo de color con números blancos", "Fundo colorido com números brancos", "خلفية ملونة وأرقام بيضاء"),
  styleNeon: row("Neon", "ניאון", "Neón", "Neon", "نيون"),
  styleNeonHint: row("Dark background with glowing accents", "רקע כהה עם הדגשות זוהרות", "Fondo oscuro con acentos brillantes", "Fundo escuro com destaques brilhantes", "خلفية داكنة وتمييز متوهج"),
  layoutCompact: row("On-page block", "רכיב בעמוד", "Bloque en la página", "Bloco na página", "مكوّن في الصفحة"),
  layoutCompactHint: row("Size and position — drag and resize in the editor like an image", "גודל ומיקום — גררו ושנו גודל בעורך כמו תמונה", "Tamaño y posición — arrastra en el editor", "Tamanho e posição — arraste no editor", "الحجم والموضع — اسحب في المحرر"),
  layoutFloating: row("Fixed floating", "צף קבוע", "Flotante fijo", "Flutuante fixo", "عائم ثابت"),
  layoutFloatingHint: row("A small live-site widget (not in the editor)", "ווידג'ט קטן צף באתר החי (לא בעריכה)", "Un widget pequeño en el sitio en vivo", "Um widget pequeno no site ao vivo", "ودجة صغيرة في الموقع الحي"),
  sizeSm: row("Small", "קטן", "Pequeño", "Pequeno", "صغير"),
  sizeMd: row("Medium", "בינוני", "Mediano", "Médio", "متوسط"),
  sizeLg: row("Large", "גדול", "Grande", "Grande", "كبير"),
  effectNone: row("None", "ללא", "Ninguno", "Nenhum", "بدون"),
  effectSparkle: row("Sparkles", "ניצוצות", "Destellos", "Brilhos", "ومضات"),
  effectGlow: row("Glow", "זוהר", "Brillo", "Brilho", "توهج"),
  effectFireworks: row("Fireworks", "זיקוקים", "Fuegos", "Fogos", "ألعاب نارية"),
  effectConfetti: row("Confetti", "קונפטי", "Confeti", "Confete", "قصاصات"),
  whenDuring: row("While running", "תוך כדי", "Durante", "Durante", "أثناء التشغيل"),
  whenExpire: row("On finish", "בסיום", "Al terminar", "Ao terminar", "عند الانتهاء"),
  whenBoth: row("Both", "שניהם", "Ambos", "Ambos", "كلاهما"),
  formatStandard: row("Standard", "רגיל", "Estándar", "Padrão", "قياسي"),
  formatStandardHint: row("Months, weeks, days — with conversions", "חודשים, שבועות, ימים — עם המרות", "Meses, semanas, días — con conversiones", "Meses, semanas, dias — com conversões", "أشهر وأسابيع وأيام مع تحويل"),
  formatDays: row("Days only", "ימים בלבד", "Solo días", "Só dias", "أيام فقط"),
  formatDaysHint: row("Total days + hours/minutes/seconds", "סה״כ ימים + שעות/דקות/שניות", "Días totales + horas/minutos/segundos", "Dias totais + horas/minutos/segundos", "إجمالي الأيام + ساعات/دقائق/ثوانٍ"),
  formatWeeks: row("Weeks only", "שבועות בלבד", "Solo semanas", "Só semanas", "أسابيع فقط"),
  formatWeeksHint: row("Total weeks + remaining units", "סה״כ שבועות + שאר יחידות", "Semanas totales + resto", "Semanas totais + resto", "إجمالي الأسابيع + الوحدات الباقية"),
  fontSystem: row("System", "רגיל", "Sistema", "Sistema", "النظام"),
  fontRounded: row("Rounded", "מעוגל", "Redondeada", "Arredondada", "مستدير"),
  fontBold: row("Bold", "מודגש", "Negrita", "Negrito", "عريض"),
  fontMono: row("Mono", "מונו", "Mono", "Mono", "أحادي"),
};
