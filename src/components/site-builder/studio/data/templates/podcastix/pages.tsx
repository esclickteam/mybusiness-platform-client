...
const allowedPages = podcastixPages.map((p) => p.id);
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 mx-3 mt-3 rounded-full border border-white/15 bg-[var(--surface)]/85 text-[var(--text)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-full bg-[var(--p)] text-sm font-black text-white">PO</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "podcastix:PageHero:11:waveform mic";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "podcastix:Hero:11:waveform mic";
  const stats = [
...
function About({ data }: { data: Record<string, any> }) {
  void "podcastix:About:11:waveform mic";
  return (
        ...
        <Reveal variant="right" className="">
          <div className="relative overflow-hidden rounded-[1rem_5rem_1rem_5rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">WAVEFORM MIC</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "podcastix:Services:11:waveform mic";
  const services = [
...
function Cases({ data }: { data: Record<string, any> }) {
  void "podcastix:Cases:11:waveform mic";
  const items = [
...
function Team({ data }: { data: Record<string, any> }) {
  void "podcastix:Team:11:waveform mic";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "podcastix:Gallery:11:waveform mic";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "podcastix:WhyUs:11:waveform mic";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "podcastix:Method:11:waveform mic";
...
function Stats({ data }: { data: Record<string, any> }) {
  void "podcastix:Stats:11:waveform mic";
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "podcastix:Outcomes:11:waveform mic";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "podcastix:Insights:11:waveform mic";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "podcastix:Pricing:11:waveform mic";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "podcastix:Faq:11:waveform mic";
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "podcastix:CTABand:11:waveform mic";
  return (
...
function Contact({ data }: { data: Record<string, any> }) {
  void "podcastix:Contact:11:waveform mic";
  return (
    <section className="bg-[var(--dark)] text-white px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[1rem_3rem_1rem_3rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="podcastix-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "podcastix:Footer:11:waveform mic";
  const links = podcastixPages.filter((p) => p.id !== "home");
  return (
...
export default function PodcastixPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...podcastixDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="podcastix" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: podcastixEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={podcastixPages.map((p) => ({
          id: p.id,
