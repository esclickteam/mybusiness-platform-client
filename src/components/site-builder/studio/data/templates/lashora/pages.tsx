...
function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  return (
    ...
    <div className="mx-auto flex max-w-7xl items-center gap-5 px-5 py-4 lg:px-8">
      ...
      />
      <button type="button" onClick={() => goTo("booking")} className="mr-auto px-4 py-2 text-xs font-bold border border-[var(--p)] text-[var(--p)]" data-editable="button" data-visual-edit-id="global.header.primaryCta" data-visual-edit-type="button" data-visual-label="כפתור ראשי בכותרת">{getValue(data,"heroPrimaryButton")}</button>
    </div>
  </header>
  );
}
