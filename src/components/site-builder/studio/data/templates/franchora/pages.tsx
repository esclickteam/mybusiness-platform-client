...
const allowedPages = franchoraPages.map((p) => p.id);
...
function getValue(data: Record<string, any>, key: string) {
  return data?.[key] ?? (franchoraDefaultData as Record<string, any>)[key] ?? "";
}

function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  void "franchora:Header:14:system map multi-location";
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 mx-3 mt-3 rounded-full border border-white/15 bg-[var(--surface)]/85 text-[var(--text)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-2xl bg-[var(--p)] text-sm font-black text-white">FR</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "franchora:PageHero:14:system map multi-location";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "franchora:Hero:14:system map multi-location";
  const stats = [
  ...
  return (
    <section data-template-section-type="hero" className="relative overflow-hidden bg-[var(--dark)] text-white">
      ...
      <div className="relative z-10 mx-auto grid min-h-[92svh] max-w-7xl items-center gap-8 px-5 py-20 lg:grid-cols-[0.7fr_1.3fr] lg:px-8">
        ...
        <Reveal variant="right" className="order-1 text-start lg:order-2">
...
function About({ data }: { data: Record<string, any> }) {
  void "franchora:About:14:system map multi-location";
  return (
        ...
        <Reveal variant="scale" className="order-2 lg:order-1">
          <div className="relative overflow-hidden rounded-[1rem_5rem_1rem_5rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">SYSTEM MAP MULTI-LOCATION</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "franchora:Services:14:system map multi-location";
  const services = [
...
function Cases({ data }: { data: Record<string, any> }) {
  void "franchora:Cases:14:system map multi-location";
  const items = [
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {items.map(([title, text, image], i) => (
            <Reveal key={title} delayMs={i * 110} variant="scale">
              <article className="ag-card relative min-h-[520px] overflow-hidden rounded-[3rem] border border-current/10 bg-[var(--surface)] text-start">
...
function Team({ data }: { data: Record<string, any> }) {
  void "franchora:Team:14:system map multi-location";
  const people = [
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {people.map(([name, role, image], i) => (
            <Reveal key={name} delayMs={i * 100} variant="up">
              <article className="ag-card overflow-hidden rounded-[2.5rem] border border-current/10 bg-[var(--surface)] text-start">
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "franchora:Gallery:14:system map multi-location";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "franchora:WhyUs:14:system map multi-location";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "franchora:Method:14:system map multi-location";
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {steps.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 100} variant="up">
              <article className="ag-card rounded-[1rem_3rem_1rem_3rem] border border-current/10 bg-[var(--surface)]/80 p-6 text-start text-[var(--text)]">
...
function Stats({ data }: { data: Record<string, any> }) {
  void "franchora:Stats:14:system map multi-location";
      ...
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-4">
        {stats.map(([value, label], i) => (
          <Reveal key={label} delayMs={i * 80} variant="up">
            <div className="ag-card rounded-[1rem_3rem_1rem_3rem] border border-current/10 bg-[var(--surface)] p-7 text-start">
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "franchora:Outcomes:14:system map multi-location";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "franchora:Insights:14:system map multi-location";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "franchora:Pricing:14:system map multi-location";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "franchora:Faq:14:system map multi-location";
        ...
        <div className="mt-10 grid gap-3">
          {faqs.map(([q, a], i) => (
            <Reveal key={q} delayMs={i * 80} variant="up">
              <article className="ag-card rounded-[1rem_3rem_1rem_3rem] border border-current/10 bg-[var(--surface)] p-6 text-start">
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "franchora:CTABand:14:system map multi-location";
  return (
    <section className="px-5 py-12 lg:px-8">
      <Reveal variant="scale" className="mx-auto max-w-7xl overflow-hidden rounded-[1rem_3rem_1rem_3rem] bg-[var(--p)] p-8 text-start text-white md:p-12">
...
function Contact({ data }: { data: Record<string, any> }) {
  void "franchora:Contact:14:system map multi-location";
  return (
    <section className="bg-[var(--surface)] px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[1rem_3rem_1rem_3rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="franchora-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "franchora:Footer:14:system map multi-location";
  const links = franchoraPages.filter((p) => p.id !== "home");
  return (
...
export default function FranchoraPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...franchoraDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="franchora" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: franchoraEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={franchoraPages.map((p) => ({
          id: p.id,
