...
export const meridianPages = [
  { id: "home", label: "בית", slug: "/" },
  { id: "rooms", label: "חדרים", slug: "/rooms" },
  { id: "experience", label: "החוויה", slug: "/experience" },
  { id: "dining", label: "מסעדה", slug: "/dining" },
  { id: "contact", label: "יצירת קשר", slug: "/contact" },
];

const allowedPages = meridianPages.map((p) => p.id);
...
function v(data: Record<string, any>, key: string) {
  return data?.[key] ?? (meridianDefaultData as Record<string, any>)[key] ?? "";
}
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {nav.map(([id, label]) => (
            <button
              ...
              style={{ color: currentPage === id ? "#f3ebe1" : "#a89a88", opacity: currentPage === id ? 1 : 0.85 }}
            >
              {label}
            </button>
          ))}
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3"  data-bizuply-block="lead-form" data-bizuply-form-id="meridian-contact-1" data-bizuply-crm-lead="true" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אליכם בהקדם." >
      <input name="name" data-bizuply-form-field-id="name" autoComplete="name"  className={field} style={{ borderColor: "rgba(243,235,225,0.14)", color: "#f3ebe1" }} placeholder="שם מלא" />
      <input name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel"  className={field} style={{ borderColor: "rgba(243,235,225,0.14)", color: "#f3ebe1" }} placeholder="טלפון" />
      <input name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email"  className={field} style={{ borderColor: "rgba(243,235,225,0.14)", color: "#f3ebe1" }} placeholder="אימייל" />
      <textarea name="message" data-bizuply-form-field-id="message"  className={cx(field, "min-h-28")} style={{ borderColor: "rgba(243,235,225,0.14)", color: "#f3ebe1" }} placeholder="ספרו בקצרה" />
      <button type="button" onClick={onCta} className="px-6 py-4 text-sm font-bold" style={{ background: "#c4a574", color: "#0A0908" }}>
        {v(data, "cta")}
      </button>
    </form>
  );
...
function Hero({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  ...
  if (layout === "type") {
    return (
      <section className="relative isolate min-h-[88vh] overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, #12100ef0 0%, #12100e88 55%, transparent 100%)" }} />
        <div className="relative z-10 mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-5 pb-16 pt-28 lg:px-8">
          <p className="text-xs font-semibold tracking-[0.28em]" style={{ color: "#c4a574" }}>{v(data, "heroEyebrow")}</p>
          <h1 className="tpl-display mt-4 max-w-4xl text-6xl font-extrabold leading-[0.95] md:text-8xl">{v(data, "heroTitle")}</h1>
          <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#a89a88" }}>{v(data, "heroSubtitle")}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#c4a574", color: "#0A0908" }}>{v(data, "heroPrimary")}</button>
            <button type="button" onClick={() => goTo("rooms")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(243,235,225,0.14)" }}>{v(data, "heroSecondary")}</button>
          </div>
        </div>
      </section>
    );
  ...
  if (layout === "poster") {
    return (
      <section className="mx-auto grid max-w-7xl gap-0 px-5 py-10 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-16">
        <div className="flex flex-col justify-between border p-8 lg:min-h-[78vh] lg:p-12" style={{ borderColor: "rgba(243,235,225,0.14)", background: "#1c1915" }}>
          <div>
            <p className="text-xs font-semibold tracking-[0.28em]" style={{ color: "#c4a574" }}>{v(data, "heroEyebrow")}</p>
            <h1 className="tpl-display mt-8 text-6xl font-bold leading-[0.95] md:text-7xl">{v(data, "heroTitle")}</h1>
          </div>
          <div>
            <p className="max-w-md text-lg leading-8" style={{ color: "#a89a88" }}>{v(data, "heroSubtitle")}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#c4a574", color: "#0A0908" }}>{v(data, "heroPrimary")}</button>
              <button type="button" onClick={() => goTo("rooms")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(243,235,225,0.14)" }}>{v(data, "heroSecondary")}</button>
            </div>
      ...
      <section className="relative isolate overflow-hidden">
        <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.05fr_0.95fr]">
          <div className="flex flex-col justify-center px-5 py-16 lg:px-8 lg:py-24">
            <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#c4a574" }}>{v(data, "heroEyebrow")}</p>
            <h1 className="tpl-display mt-5 text-5xl font-bold leading-[1.02] md:text-7xl">{v(data, "heroTitle")}</h1>
            <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#a89a88" }}>{v(data, "heroSubtitle")}</p>
            <div className="mt-9 flex flex-wrap gap-3">
              <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#c4a574", color: "#f7f3ee" }}>{v(data, "heroPrimary")}</button>
              <button type="button" onClick={() => goTo("rooms")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(243,235,225,0.14)" }}>{v(data, "heroSecondary")}</button>
            </div>
          </div>
  ...
  return (
    <section className="relative isolate min-h-[86vh] overflow-hidden">
      <img src={v(data, "heroImage")} alt="" className="absolute inset-0 h-full w-full object-cover" />
      <div className="absolute inset-0" style={{ background: "linear-gradient(105deg, #12100ef2 0%, #12100e99 48%, transparent 78%)" }} />
      <div className="relative z-10 mx-auto flex min-h-[86vh] max-w-7xl items-end px-5 pb-16 pt-28 lg:px-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-6 max-w-lg text-lg leading-8" style={{ color: "#a89a88" }}>{v(data, "heroSubtitle")}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#c4a574", color: "#0A0908" }}>{v(data, "heroPrimary")}</button>
            <button type="button" onClick={() => goTo("rooms")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(243,235,225,0.14)" }}>{v(data, "heroSecondary")}</button>
          </div>
    ...
    <section className="border-t" style={{ borderColor: "rgba(243,235,225,0.14)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-12 lg:py-24">
          ...
          <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#a89a88" }}>{v(data, "aboutText")}</p>
        </div>
  ...
  return (
    <>
      <section className="border-b px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(243,235,225,0.14)" }}>
        <div className="mx-auto max-w-7xl">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#c4a574" }}>{v(data, "brandName")}</p>
...
export default function MeridianPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const merged = useMemo(() => ({ ...meridianDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div
      dir="rtl"
      data-template-id={mode === "preview" ? "meridian-preview" : "meridian"}
