...
export const gelatixPages = [
  { id: "home", label: "בית", slug: "/" },
  { id: "scoops", label: "טעמים", slug: "/scoops" },
  { id: "lab", label: "המעבדה", slug: "/lab" },
  { id: "about", label: "אודות", slug: "/about" },
  { id: "contact", label: "הזמנה", slug: "/contact" },
];

const allowedPages = gelatixPages.map((p) => p.id);
...
function v(data: Record<string, any>, key: string) {
  return data?.[key] ?? (gelatixDefaultData as Record<string, any>)[key] ?? "";
}
        ...
        <nav className="hidden items-center gap-5 lg:flex">
          {nav.map(([id, label]) => (
            <button key={id} type="button" onClick={() => goTo(id)} className="text-sm font-semibold"
              style={{ color: currentPage === id ? "#2b1822" : "#9a6b7c" }}>{label}</button>
          ))}
        </nav>
        <button type="button" onClick={onCta} className="rounded-full px-4 py-2 text-xs font-bold" style={{ background: "#e85a8c", color: "#ffffff" }}>{v(data, "heroPrimary")}</button>
      </div>
    </header>
  );
}
  ...
  return (
      <section className="relative isolate min-h-[92vh] overflow-hidden pt-20">
        ...
        <div className="relative z-10 mx-auto grid min-h-[92vh] max-w-7xl items-center gap-10 px-5 py-12 md:py-24 lg:grid-cols-2 lg:px-8">
          ...
          <p className="tpl-rise-3 mt-6 max-w-xl text-lg leading-8" style={{ color: "#9a6b7c" }}>{v(data, "heroSubtitle")}</p>
          <div className="tpl-rise-3 mt-8 flex flex-wrap gap-3">
            <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#e85a8c", color: "#ffffff" }}>{v(data, "heroPrimary")}</button>
            <button type="button" onClick={() => goTo("scoops")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(43,24,34,0.1)" }}>{v(data, "heroSecondary")}</button>
          </div></div>
          <div className="tpl-scoop mx-auto aspect-square w-full max-w-md overflow-hidden rounded-full border-8" style={{ borderColor: "#e85a8c" }}>
...
function MeltScoopRing({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="relative border-t px-5 py-16 lg:px-8 lg:py-20 overflow-hidden" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
      <div className="tpl-drip pointer-events-none absolute left-1/4 top-0 h-24 w-8 rounded-b-full opacity-40" style={{ background: "#e85a8c" }} />
      <div className="tpl-drip pointer-events-none absolute right-1/3 top-0 h-16 w-6 rounded-b-full opacity-30" style={{ background: "#e85a8c", animationDelay: ".8s" }} />
          ...
          {cards.map((c, i) => (
            <Reveal key={c.title} delayMs={i * 100} variant="up">
              <article className="w-40 md:w-48">
                <div className="tpl-scoop mx-auto aspect-square overflow-hidden rounded-full border-4" style={{ borderColor: "#e85a8c" }}>
                  <img src={c.img} alt="" className="h-full w-full object-cover" />
                </div>
                <p className="mt-4 text-xs" style={{ color: "#e85a8c" }}>{c.meta}</p>
                <h3 className="tpl-display mt-1 text-xl font-bold">{c.title}</h3>
                <p className="mt-2 text-sm" style={{ color: "#9a6b7c" }}>{c.text}</p>
              </article>
            </Reveal>
          ))}
        </div>
...
function MeltProcessSteps({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
        ...
        <ol className="mt-10 space-y-0">
          {steps.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 70} variant="right">
              <li className="grid grid-cols-[48px_1fr] gap-4 border-r-2 pr-4 pb-8" style={{ borderColor: i < 2 ? "#e85a8c" : "transparent" }}>
                <span className="tpl-display text-3xl font-bold" style={{ color: "#e85a8c" }}>{i + 1}</span>
                <div>
                  <h3 className="tpl-display text-xl font-bold">{t}</h3>
                  <p className="mt-2 text-sm leading-7" style={{ color: "#9a6b7c" }}>{x}</p>
                </div>
              </li>
            </Reveal>
          ))}
...
function MeltHomeGallery({ data }: { data: Record<string, any> }) {
...
function MeltHomeReviews({ data }: { data: Record<string, any> }) {
...
function MeltHomeStats({ data }: { data: Record<string, any> }) {
  const stats = [[v(data, "stat1"), v(data, "stat1Label")], [v(data, "stat2"), v(data, "stat2Label")], [v(data, "stat3"), v(data, "stat3Label")]];
  return (
    <section className="border-t px-5 py-12 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#1a0f14", color: "#2b1822" }}>
      <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-8">
        ...
        <p className="w-full text-center text-sm" style={{ color: "#9a6b7c" }}>{v(data, "hours")}</p>
      </div>
    </section>
  );
}

function MeltHomeCtaTeaser({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 border p-8 md:flex-row md:items-center" style={{ borderColor: "#e85a8c", background: "#ffffff" }}>
        <div>
...
function MeltSpecialtyBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="px-5 py-20 text-center lg:px-8" style={{ background: "#1a0f14", color: "#2b1822" }}>
      <p className="text-xs tracking-[0.3em]" style={{ color: "#e85a8c" }}>{v(data, "brandName")}</p>
      <h1 className="tpl-display mt-4 text-5xl font-bold md:text-7xl">{v(data, "page1Title")}</h1>
      <p className="mx-auto mt-4 max-w-xl text-lg" style={{ color: "#9a6b7c" }}>{v(data, "page1Subtitle")}</p>
    </section>
  );
}
function MeltFullMenuBoard({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-10 divide-y" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
...
function MeltChefPicks({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
          <p className="text-xs tracking-[0.24em]" style={{ color: "#e85a8c" }}>{v(data, "chefPickEyebrow")}</p>
          <h2 className="tpl-display mt-3 text-3xl font-bold">{v(data, "chefPickTitle")}</h2>
          <p className="mt-4 text-lg leading-8" style={{ color: "#9a6b7c" }}>{v(data, "chefPickText")}</p>
        </div>
...
function MeltCategoryGrid({ data }: { data: Record<string, any> }) {
      ...
      <div className="mx-auto grid max-w-7xl gap-3 md:grid-cols-4">
        {cats.map(([t, x], i) => (
          <Reveal key={t} delayMs={i * 70} variant="up">
            <div className="border p-5" style={{ borderColor: "rgba(43,24,34,0.1)", background: i % 2 ? "#ffffff" : "#fff5f8" }}>
              <h3 className="tpl-display text-xl font-bold" style={{ color: "#e85a8c" }}>{t}</h3>
              <p className="mt-2 text-sm" style={{ color: "#9a6b7c" }}>{x}</p>
            </div>
          </Reveal>
        ))}
      </div>
...
function MeltPairingNotes({ data }: { data: Record<string, any> }) {
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {pairs.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 80} variant="up">
              <div className="border-r pr-4" style={{ borderColor: "#e85a8c" }}>
...
function MeltStoryBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="relative overflow-hidden px-5 py-20 lg:px-8" style={{ background: "#ffffff" }}>
      <div className="mx-auto max-w-7xl">
        <p className="text-xs tracking-[0.28em]" style={{ color: "#e85a8c" }}>{v(data, "brandName")}</p>
        <h1 className="tpl-display mt-4 max-w-3xl text-5xl font-bold md:text-6xl">{v(data, "page2Title")}</h1>
        <p className="mt-4 max-w-xl text-lg" style={{ color: "#9a6b7c" }}>{v(data, "page2Subtitle")}</p>
      </div>
    </section>
  );
}
function MeltTechniqueLadder({ data }: { data: Record<string, any> }) {
    ...
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-10 space-y-6">
          {steps.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 70} variant="up">
              <div className="grid gap-2 border p-5 md:grid-cols-[60px_1fr]" style={{ borderColor: "rgba(43,24,34,0.1)", background: i % 2 ? "#ffffff" : "#fff5f8" }}>
                <span className="tpl-display text-3xl font-bold" style={{ color: "#e85a8c" }}>0{i + 1}</span>
                <div>
                  <h3 className="tpl-display text-xl font-bold">{t}</h3>
                  <p className="mt-2 text-sm leading-7" style={{ color: "#9a6b7c" }}>{x}</p>
                </div>
...
function MeltMaterialCards({ data }: { data: Record<string, any> }) {
  const mats = [[v(data, "mat1Title"), v(data, "mat1Text")], [v(data, "mat2Title"), v(data, "mat2Text")], [v(data, "mat3Title"), v(data, "mat3Text")]];
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
      <div className="mx-auto max-w-7xl">
        <Reveal><h2 className="tpl-display text-3xl font-bold">{v(data, "matTitle")}</h2></Reveal>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {mats.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 70} variant="up">
              <div className="border p-6" style={{ borderColor: "#e85a8c" }}>
                <h3 className="tpl-display text-xl font-bold">{t}</h3>
                <p className="mt-3 text-sm" style={{ color: "#9a6b7c" }}>{x}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
function MeltEventsBand({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2 lg:items-center">
        ...
        <div>
          ...
          <p className="mt-4 text-lg leading-8" style={{ color: "#9a6b7c" }}>{v(data, "eventsText")}</p>
          <p className="mt-4 text-sm font-semibold" style={{ color: "#e85a8c" }}>{v(data, "eventsMeta")}</p>
        </div>
      </div>
    </section>
  );
...
function MeltAboutBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-b px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto max-w-7xl">
        <p className="text-xs tracking-[0.28em]" style={{ color: "#e85a8c" }}>{v(data, "aboutEyebrow")}</p>
        <h1 className="tpl-display mt-4 text-5xl font-bold md:text-6xl">{v(data, "aboutPageTitle")}</h1>
        <p className="mt-4 max-w-2xl text-lg leading-8" style={{ color: "#9a6b7c" }}>{v(data, "aboutPageLead")}</p>
      </div>
    </section>
  );
}
function MeltAboutTimeline({ data }: { data: Record<string, any> }) {
    ...
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
      <div className="mx-auto max-w-3xl">
        ...
        <div className="relative mt-10">
          <div className="absolute right-3 top-0 bottom-0 w-px" style={{ background: "rgba(43,24,34,0.1)" }} />
...
function MeltChefPortrait({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
...
function MeltValuesRow({ data }: { data: Record<string, any> }) {
    ...
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-3">
        {vals.map(([t, x], i) => (
          <Reveal key={t} delayMs={i * 70} variant="up">
            <div className="p-5">
              <h3 className="tpl-display text-2xl font-bold" style={{ color: "#e85a8c" }}>{t}</h3>
              <p className="mt-3 text-sm leading-7" style={{ color: "#9a6b7c" }}>{x}</p>
            </div>
          </Reveal>
        ))}
      </div>
...
function MeltContactBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-b px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto max-w-7xl">
        <p className="text-xs tracking-[0.28em]" style={{ color: "#e85a8c" }}>{v(data, "contactEyebrow")}</p>
        <h1 className="tpl-display mt-4 text-5xl font-bold md:text-6xl">{v(data, "contactPageTitle")}</h1>
        <p className="mt-4 max-w-xl text-lg" style={{ color: "#9a6b7c" }}>{v(data, "contactPageText")}</p>
      </div>
    </section>
  );
}
function MeltReserveForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
      <div className="mx-auto max-w-lg">
        ...
        <div className="mt-8 space-y-3">
          ...
          <form className="ml-8 grid gap-2 border p-3" style={{ borderColor: "#e85a8c" }}  data-bizuply-block="lead-form" data-bizuply-form-id="gelatix-contact-1" data-bizuply-crm-lead="true" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אליכם בהקדם." >
            <input name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel"  className="w-full border bg-transparent px-3 py-2.5 text-right text-sm outline-none" style={{ borderColor: "rgba(43,24,34,0.1)", color: "#2b1822" }} placeholder="שם + טלפון" />
...
function MeltHoursMap({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">
        <div className="border p-6" style={{ borderColor: "rgba(43,24,34,0.1)", background: "#ffffff" }}>
          <h3 className="tpl-display text-2xl font-bold">{v(data, "hoursTitle")}</h3>
          <p className="mt-4 text-sm leading-7" style={{ color: "#9a6b7c" }}>{v(data, "hours")}</p>
          <p className="mt-4 text-sm">{v(data, "address")}</p>
        </div>
        <div className="relative min-h-[220px] overflow-hidden border" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
          <img src={v(data, "galleryImage1")} alt="" className="absolute inset-0 h-full w-full object-cover opacity-60" />
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="border px-4 py-2 text-xs font-bold tracking-wider" style={{ borderColor: "#e85a8c", background: "#fff5f8", color: "#e85a8c" }}>{v(data, "mapLabel")}</span>
          </div>
        </div>
      </div>
    </section>
...
function MeltFaqBlock({ data }: { data: Record<string, any> }) {
      ...
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        {faqs.map(([q, a], i) => (
          <Reveal key={q} delayMs={i * 70} variant="up">
            <details className="border p-4" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
              <summary className="cursor-pointer font-bold">{q}</summary>
              <p className="mt-3 text-sm leading-7" style={{ color: "#9a6b7c" }}>{a}</p>
            </details>
          </Reveal>
        ))}
      </div>
...
function Footer({ data }: { data: Record<string, any> }) {
  return (
    <footer className="border-t px-5 py-10 lg:px-8" style={{ borderColor: "rgba(43,24,34,0.1)" }}>
      <div className="mx-auto max-w-5xl text-center">
        <div className="tpl-drip mx-auto mb-4 h-8 w-4 rounded-b-full" style={{ background: "#e85a8c" }} />
        <span className="tpl-display text-2xl font-bold">{v(data, "brandName")}</span>
        <p className="mt-2 text-sm" style={{ color: "#9a6b7c" }}>{v(data, "email")}</p>
      </div>
    </footer>
  );
}
...
function HomePage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      ...
      <MeltScoopRing data={data} />
      <MeltProcessSteps data={data} />
      <MeltHomeGallery data={data} />
      <MeltHomeReviews data={data} />
      <MeltHomeStats data={data} />
      <MeltHomeCtaTeaser data={data} goTo={goTo} />
      <Footer data={data} />
    </>
  );
}

function ScoopsPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <MeltSpecialtyBanner data={data} />
      <MeltFullMenuBoard data={data} />
      <MeltChefPicks data={data} />
      <MeltCategoryGrid data={data} />
      <MeltPairingNotes data={data} />
      <Footer data={data} />
    </>
  );
}

function LabPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <MeltStoryBanner data={data} />
      <MeltTechniqueLadder data={data} />
      <MeltMaterialCards data={data} />
      <MeltEventsBand data={data} />
      <Footer data={data} />
    </>
  );
}
...
function AboutPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <MeltAboutBanner data={data} />
      <MeltAboutTimeline data={data} />
      <MeltChefPortrait data={data} />
      <MeltValuesRow data={data} />
      <Footer data={data} />
    </>
  );
}
...
function ContactPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <MeltContactBanner data={data} />
      <MeltReserveForm data={data} onCta={onCta} />
      <MeltHoursMap data={data} />
      <MeltFaqBlock data={data} />
      <Footer data={data} />
    </>
  );
}

export default function GelatixPages({
  initialPage = "home", initialPageId, page, pageId, activePageId, currentPageId,
  mode = "preview", data, onPageChange, isPublic, viewMode, runtimeMode,
}: Props) {
  const merged = useMemo(() => ({ ...gelatixDefaultData, ...(data ?? {}) }), [data]);
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    scoops: <ScoopsPage data={merged} goTo={goTo} onCta={() => goTo("contact")} />,
    lab: <LabPage data={merged} goTo={goTo} onCta={() => goTo("contact")} />,
    about: <AboutPage data={merged} goTo={goTo} onCta={() => goTo("contact")} />,
    contact: <ContactPage data={merged} goTo={goTo} onCta={() => goTo("contact")} />,
  };
  return (
    <div dir="rtl" data-template-id={mode === "preview" ? "gelatix-preview" : "gelatix"} className="min-h-screen w-full overflow-x-hidden"
