...
const allowedPages = launchoraPages.map((p) => p.id);
...
function getValue(data: Record<string, any>, key: string) {
  return data?.[key] ?? (launchoraDefaultData as Record<string, any>)[key] ?? "";
}

function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  void "launchora:Header:6:countdown ignition";
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 rounded-b-[2rem] border-b border-white/15 bg-[var(--dark)]/90 text-white backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-2xl bg-[var(--p)] text-sm font-black text-white">LA</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "launchora:PageHero:6:countdown ignition";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "launchora:Hero:6:countdown ignition";
  const stats = [
...
function About({ data }: { data: Record<string, any> }) {
  void "launchora:About:6:countdown ignition";
  return (
        ...
        <Reveal variant="scale" className="order-2 lg:order-1">
          <div className="relative overflow-hidden rounded-[3rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">COUNTDOWN IGNITION</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "launchora:Services:6:countdown ignition";
  const services = [
...
function Cases({ data }: { data: Record<string, any> }) {
  void "launchora:Cases:6:countdown ignition";
  const items = [
...
function Team({ data }: { data: Record<string, any> }) {
  void "launchora:Team:6:countdown ignition";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "launchora:Gallery:6:countdown ignition";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "launchora:WhyUs:6:countdown ignition";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "launchora:Method:6:countdown ignition";
...
function Stats({ data }: { data: Record<string, any> }) {
  void "launchora:Stats:6:countdown ignition";
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "launchora:Outcomes:6:countdown ignition";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "launchora:Insights:6:countdown ignition";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "launchora:Pricing:6:countdown ignition";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "launchora:Faq:6:countdown ignition";
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "launchora:CTABand:6:countdown ignition";
  return (
...
function Contact({ data }: { data: Record<string, any> }) {
  void "launchora:Contact:6:countdown ignition";
  return (
    <section className="bg-[var(--surface)] px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[2rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="launchora-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "launchora:Footer:6:countdown ignition";
  const links = launchoraPages.filter((p) => p.id !== "home");
  return (
...
export default function LaunchoraPages({
  ...
  const mergedData = useMemo(() => ({ ...launchoraDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="launchora" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: launchoraEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={launchoraPages.map((p) => ({
          id: p.id,
