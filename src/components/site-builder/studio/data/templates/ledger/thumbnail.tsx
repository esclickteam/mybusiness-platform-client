...
export default function LedgerThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-5" style={{ background: "#f6f3ea", color: "#102018" }}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "#0d5c45" }}>
          {tx("פיננסים והנהלת חשבונות")}</span>
      </div>
      <div className="mt-10">
        <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.24em]" style={{ color: "#0d5c45" }}>
          {tx("פיננסים והנהלת חשבונות")}</p>
        <h3 className="max-w-[220px] text-2xl font-black leading-tight">{tx("מספרים עם הקשר עסקי.")}</h3>
