...
export default function PawhausThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-5" style={{ background: "#FAF7F2", color: "#1C1917" }}>
      <div className="flex items-center justify-between">
        <div className="grid h-9 w-9 place-items-center text-sm font-black text-white" style={{ background: "#F59E0B" }}>P</div>
        <span className="text-[10px] font-black tracking-[0.28em]" style={{ color: "#F59E0B" }}>{tx("טיפול בחיות")}</span>
      </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2">
        {[tx("מעון"), tx("פנסיון"), tx("טיפוח")].map((label) => (
