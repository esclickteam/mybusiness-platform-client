      ...
      <div className="mx-auto mt-8 max-w-4xl space-y-4">
        {[[getValue(data,"itemOneTitle"),getValue(data,"itemOneText")],[getValue(data,"itemTwoTitle"),getValue(data,"itemTwoText")],[getValue(data,"itemThreeTitle"),getValue(data,"itemThreeText")]].map(([title,text],i) => (
          <Reveal key={title} delayMs={i*80} className="t-hover border border-[var(--p)]/30 bg-[var(--surface)] p-5">
            ...
            <p className="text-white/80">"desc": "{tx(text)}",</p>
...
function Curriculum({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="curriculum" className="border-y border-[var(--p)]/20 bg-[var(--surface)] px-5 py-12 md:py-24 font-mono lg:px-8">
...
function Instructors({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="instructors" className="px-5 py-12 md:py-24 font-mono lg:px-8">
...
function Testimonials({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="testimonials" className="px-5 py-12 md:py-24 font-mono lg:px-8">
      ...
      <div className="mx-auto mt-8 max-w-3xl space-y-4">
        {[[getValue(data,"reviewOneText"),getValue(data,"reviewOneName"),getValue(data,"reviewOneRole")],[getValue(data,"reviewTwoText"),getValue(data,"reviewTwoName"),getValue(data,"reviewTwoRole")],[getValue(data,"reviewThreeText"),getValue(data,"reviewThreeName"),getValue(data,"reviewThreeRole")]].map(([text,name,role],i) => (
          <Reveal key={name} delayMs={i*80} className="t-hover border border-[var(--p)]/30 bg-[var(--surface)] p-5">
            ...
            <p className="mt-2 text-white/90">"{tx(text)}"</p>
...
function Faq({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="faq" className="bg-[var(--surface)] px-5 py-12 md:py-24 font-mono lg:px-8">
...
function PageHero({ data, page, goTo }: PageProps & { page: PageEntry }) {
  return (
    <section data-template-section-type="pageHero" className="relative overflow-hidden px-5 py-14 md:py-28 lg:px-8 bg-black font-mono text-white">
...
function About({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="about" className="px-5 py-12 md:py-24 lg:px-8 bg-black font-mono text-white">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
        <Reveal className="relative min-h-[360px] overflow-hidden border border-[var(--p)]/35 bg-[var(--surface)]">
          ...
          <span className="absolute bottom-6 right-6 rounded-full px-4 py-2 text-sm font-bold border border-[var(--p)] text-[var(--p)]">{tx("קוד")}</span>
        </Reveal>
...
function WhyUs({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="why" className="px-5 py-12 md:py-24 lg:px-8 bg-black font-mono text-white">
...
function Method({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="method" className="px-5 py-12 md:py-24 lg:px-8 bg-black font-mono text-white">
      <Reveal className="mx-auto max-w-3xl text-center">
...
function Gallery({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="gallery" className="px-5 py-12 md:py-24 lg:px-8 bg-black font-mono text-white">
      <Reveal className="mx-auto max-w-3xl text-center">
...
function Pricing({ data, goTo }: Pick<PageProps, "data" | "goTo">) {
  return (
    <section data-template-section-type="pricing" className="px-5 py-12 md:py-24 lg:px-8 bg-black font-mono text-white">
      <Reveal className="mx-auto max-w-3xl text-center">
...
function Insights({ data, goTo }: Pick<PageProps, "data" | "goTo">) {
  return (
    <section data-template-section-type="insights" className="px-5 py-12 md:py-24 lg:px-8 bg-black font-mono text-white">
...
function Contact({ data, openModal }: { data: Record<string, any>; openModal: () => void }) {
  return (
    <section data-template-section-type="contact" className="px-5 py-12 md:py-24 font-mono lg:px-8">
      <div className="mx-auto max-w-xl border border-[var(--p)]/40 bg-black p-6">
        ...
        <form className="mt-6 grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="codehaus-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
          <input className="border border-[var(--p)]/30 bg-transparent px-4 py-3 text-[var(--p)] outline-none" placeholder="name ="  name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
          <input className="border border-[var(--p)]/30 bg-transparent px-4 py-3 text-[var(--p)] outline-none" placeholder="email ="  name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
          <button type="submit" onClick={openModal} className="bg-[var(--p)] py-3 font-bold text-black">node submit.js</button>
        </form>
      </div>
    </section>
  );
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-black/70 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md border border-[var(--p)]/30 bg-[var(--surface)] p-8">
        ...
        <form className="mt-6 grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="codehaus-contact-2" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
export default function CodehausPages(props: CodehausPagesProps) {
  ...
  return (
    <div dir={templateDir()} data-template-id="codehaus" className="min-h-screen w-full overflow-x-hidden">
