...
[data-template-id="villaire"], [data-template-id="villaire-preview"] {
  ...
  --tpl-line: rgba(244,239,230,0.12); --tpl-dark: #000000;
}
...
@keyframes villaire-ken { 0% { transform: scale(1); } 100% { transform: scale(1.08); } }
@keyframes villaire-rise { from { opacity: 0; transform: translateY(28px); } to { opacity: 1; transform: translateY(0); } }
...
@keyframes villaire-pulse-line { 0%,100% { opacity: .35; } 50% { opacity: 1; } }
@keyframes villaire-float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
@keyframes villaire-sweep { 0% { transform: translateX(-120%); } 100% { transform: translateX(120%); } }
@keyframes villaire-climb { from { transform: translateY(40px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }

[data-template-id="villaire"] .tpl-ken, [data-template-id="villaire-preview"] .tpl-ken {
  animation: villaire-ken 18s ease-in-out infinite alternate; transform-origin: center;
}
...
[data-template-id="villaire"] .tpl-sweep::after, [data-template-id="villaire-preview"] .tpl-sweep::after {
  ...
  animation: villaire-sweep 4.5s ease-in-out infinite;
}
[data-template-id="villaire"] .tpl-climb, [data-template-id="villaire-preview"] .tpl-climb {
  animation: villaire-climb .85s cubic-bezier(.22,1,.36,1) both;
}
`;
