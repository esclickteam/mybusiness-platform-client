...
function Header({ data, currentPage, goTo, onCta }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void; onCta: () => void }) {
  ...
  const nav = tidehausPages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
  return (
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="tidehaus-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(12,42,58,0.12)", color: "#0c2a3a" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(12,42,58,0.12)", color: "#0c2a3a" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(12,42,58,0.12)", color: "#0c2a3a" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(12,42,58,0.12)", color: "#0c2a3a" }} placeholder={tx("מה אתם מחפשים?")}  name="message" data-bizuply-form-field-id="message"></textarea>
      <button type="submit" className="px-6 py-4 text-sm font-bold" style={{ background: "#0077b6", color: "#ffffff" }}>{v(data, "cta")}</button>
    </form>
  );
}
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(12,42,58,0.12)", background: "#ffffff" }}>
      <div className="mx-auto max-w-7xl">
        <h2 className="tpl-display text-4xl font-bold md:text-5xl">{tx("מסילת גלשנים")}</h2>
...
function WaveForecast({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-12 lg:px-8" style={{ borderColor: "rgba(12,42,58,0.12)" }}>
      <div className="mx-auto max-w-7xl">
        <h3 className="tpl-display text-2xl font-bold">{tx("תחזית גלים היום")}</h3>
    ...
    <section className="border-t" style={{ borderColor: "rgba(12,42,58,0.12)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-8 lg:py-20">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#0077b6" }}>{tx("אודות")}</p>
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(12,42,58,0.12)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        <div>
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#0077b6" }}>{tx("יצירת קשר")}</p>
...
export default function TidehausPages({
  ...
  return (
    <div dir={templateDir()} data-template-id="tidehaus" className="min-h-screen w-full overflow-x-hidden"
