...
export default function VinoraThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative flex h-full min-h-[260px] w-full flex-col justify-between overflow-hidden p-5 text-right" style={{ background: "#FFFBF5", color: "#3F1D12", fontFamily: "Heebo, sans-serif" }}>
      ...
      <div className="relative">
        <div className="inline-flex px-2 py-1 text-[10px] font-black uppercase tracking-[0.2em]" style={{ background: "#7F1D1D", color: "#FFFBEB" }}>{tx("יקב ויינות")}</div>
        <h3 className="mt-4 text-3xl font-black leading-none" style={{ fontFamily: "Playfair Display, serif" }}>Vinora</h3>
        <p className="mt-2 text-xs font-semibold opacity-70">{tx("מרתף נבחר. כוס מדויקת.")}</p>
      </div>
