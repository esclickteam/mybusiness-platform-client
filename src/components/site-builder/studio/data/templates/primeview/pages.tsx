...
function Header({ data, currentPage, goTo, onCta }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void; onCta: () => void }) {
  ...
  const nav = primeviewPages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
  return (
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="primeview-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(15,23,42,0.1)", color: "#0f172a" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(15,23,42,0.1)", color: "#0f172a" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(15,23,42,0.1)", color: "#0f172a" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(15,23,42,0.1)", color: "#0f172a" }} placeholder={tx("מה אתם מחפשים?")}  name="message" data-bizuply-form-field-id="message"></textarea>
      <button type="submit" className="px-6 py-4 text-sm font-bold" style={{ background: "#059669", color: "#ffffff" }}>{v(data, "cta")}</button>
    </form>
  );
}
...
function SlideGalleryWall({ data }: { data: Record<string, any> }) {
  return (
...
function SlideAgentRoster({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(15,23,42,0.1)", background: "#ffffff" }}>
      <div className="mx-auto max-w-7xl divide-y" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
        {agents.map((a) => (
          <div key={a.n} className="flex flex-wrap items-center gap-5 py-6">
            ...
            <div><h3 className="tpl-display text-xl font-bold">{a.n}</h3><p style={{ color: "#059669" }}>{a.r} · {a.d}</p></div>
          </div>
        ))}
      </div>
    </section>
...
function SlideQuoteRail({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
      <div className="mx-auto max-w-3xl border-r-4 pr-8" style={{ borderColor: "#059669" }}>
        <p className="tpl-display text-3xl font-bold leading-snug">{v(data, "quote")}</p>
        <p className="mt-4 text-sm" style={{ color: "#64748b" }}>— {v(data, "brandName")}</p>
      </div>
    </section>
  );
}
function SlideTrustMetrics({ data }: { data: Record<string, any> }) {
...
function SlideMarketPulse({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(15,23,42,0.1)", background: "#ffffff" }}>
      <h2 className="tpl-display mx-auto max-w-7xl text-2xl sm:text-4xl font-bold">{tx("תובנות Slide")}</h2>
      <div className="mx-auto mt-10 grid max-w-7xl gap-5 lg:grid-cols-3">
        {posts.map(([t,x,g]) => (
          <article key={t} className="border p-5" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
            <span className="text-[10px] font-bold" style={{ color: "#059669" }}>{g}</span>
            <h3 className="tpl-display mt-2 text-xl font-bold">{t}</h3>
            <p className="mt-2 text-sm leading-7" style={{ color: "#64748b" }}>{x}</p>
          </article>
        ))}
      </div>
    </section>
...
function SlideCtaRibbon({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  return (
    <section className="px-5 py-14 lg:px-8" style={{ background: "#059669" }}>
...
function SlideFaqPanel({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
      <div className="mx-auto max-w-3xl grid gap-2">
        {faqs.map(([q,a]) => (
          <details key={q} className="border" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
            <summary className="cursor-pointer px-4 py-3 font-bold">{q}</summary>
            <p className="border-t px-4 py-3 text-sm leading-7" style={{ borderColor: "rgba(15,23,42,0.1)", color: "#64748b" }}>{a}</p>
          </details>
        ))}
      </div>
    </section>
...
function SlideOfficeBlock({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(15,23,42,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2">
        <div>
          <h2 className="tpl-display text-3xl font-bold">{v(data,"officeTitle")}</h2>
          <p className="mt-4 leading-8" style={{ color: "#64748b" }}>{v(data,"officeText")}</p>
          <p className="mt-6 text-sm font-semibold">{v(data,"phone")} · {v(data,"email")}</p>
        </div>
        <div className="relative min-h-[280px] overflow-hidden border" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
...
function SlideAwardsLane({ data }: { data: Record<string, any> }) {
  const awards = [v(data,"award1"),v(data,"award2"),v(data,"award3"),v(data,"award4")];
  return (
    <section className="overflow-hidden border-y py-4" style={{ borderColor: "rgba(15,23,42,0.1)", background: "#f8fafc" }}>
      <div className="tpl-marquee-track gap-10 px-6 text-xs font-bold tracking-[0.25em]" style={{ color: "#059669" }}>
...
function SlideProcessRail({ data }: { data: Record<string, any> }) {
...
function SlideListingGrid({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(15,23,42,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-2 lg:grid-cols-4">
        {cards.map((c) => (
          <article key={c.t} className="overflow-hidden border" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
            <img src={c.img} alt="" className="aspect-[4/3] w-full object-cover" />
            <div className="p-4"><p className="text-xs" style={{ color: "#059669" }}>{c.m}</p><h3 className="font-bold">{c.t}</h3><p className="mt-2 font-bold" style={{ color: "#059669" }}>{c.p}</p></div>
          </article>
        ))}
      </div>
    </section>
...
function SlideStatsRow({ data }: { data: Record<string, any> }) {
  const stats = [["stat1Value","stat1Label"],["stat2Value","stat2Label"],["stat3Value","stat3Label"]];
  return (
    ...
    <section className="border-t" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-8 lg:py-20">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#059669" }}>{tx("אודות")}</p>
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(15,23,42,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        <div>
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#059669" }}>{tx("יצירת קשר")}</p>
...
function HomePage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      ...
      <SlideGalleryWall data={data} />
      <SlideAwardsLane data={data} />
      <SlideQuoteRail data={data} />
      <SlideCtaRibbon data={data} onCta={onCta} />
...
export default function PrimeviewPages({
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    contact: (
      <>
        ...
        <SlideOfficeBlock data={merged} />
      <SlideAgentRoster data={merged} />
      <SlideFaqPanel data={merged} />
      <SlideTrustMetrics data={merged} />
      <SlideAwardsLane data={merged} />
        <Footer data={merged} />
      </>
    ),
  };
    ...
    pageContent["transformations"] = (
      <InnerPage data={merged} title={tx("שיפוצים")} onCta={() => goTo("contact")}>
        <>
          <BeforeAfterShowcase data={merged} />
      <ValueBands data={merged} />
      <SlideGalleryWall data={merged} />
  ...
  return (
    <div dir={templateDir()} data-template-id="primeview" className="min-h-screen w-full overflow-x-hidden"
