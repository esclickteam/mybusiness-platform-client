...
export default function ArboraThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden" style={{ background: "#eef2ea", color: "#1c2618" }}>
      ...
      <div className="relative z-10 flex h-full min-h-[260px] flex-col justify-end p-5">
        <p className="text-[10px] font-semibold tracking-[0.22em]" style={{ color: "#4f6b45" }}>{tx("אדריכלות נוף")}</p>
        <h3 className="mt-2 text-3xl font-bold leading-none" style={{ fontFamily: '"Assistant", serif' }}>Arbora</h3>
        <p className="mt-2 max-w-[220px] text-xs leading-5 opacity-80">{tx("ארבורה מתכננת חצרות, גגות ומרחבים ציבוריים — עם …")}</p>
      </div>
    </div>
  );
}
