...
function Header({
  ...
  const nav = arboraPages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || p.label] as const);
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-id="arbora-contact" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.">
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder="שם מלא" name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder="טלפון" name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder="אימייל" name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder="ספרו בקצרה"  name="message" data-bizuply-form-field-id="message"></textarea>
      ...
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1fr]">
        <div>
          ...
          <div className="mt-10 space-y-4 text-sm">
            <p><span style={{ color: "#6b7564" }}>טלפון · </span>{v(data, "phone")}</p>
            <p><span style={{ color: "#6b7564" }}>אימייל · </span>{v(data, "email")}</p>
            <p><span style={{ color: "#6b7564" }}>כתובת · </span>{v(data, "address")}</p>
          </div>
        </div>
  ...
  return (
    <footer className="border-t px-5 py-8 lg:px-8" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-3 text-sm md:flex-row md:items-center" style={{ color: "#6b7564" }}>
        ...
        <p>תבנית Arbora · Bizuply Studio</p>
      </div>
    </footer>
  );
}
...
export default function ArboraPages({
  ...
  return (
    <div
      dir="rtl"
      data-template-id={mode === "preview" ? "arbora-preview" : "arbora"}
