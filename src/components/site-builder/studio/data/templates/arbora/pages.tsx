...
export const arboraPages = [
  { id: "home", label: "בית", slug: "/" },
  { id: "projects", label: "פרויקטים", slug: "/projects" },
  { id: "approach", label: "גישה", slug: "/approach" },
  { id: "studio", label: "הסטודיו", slug: "/studio" },
  { id: "contact", label: "יצירת קשר", slug: "/contact" },
];

const allowedPages = arboraPages.map((p) => p.id);
...
function v(data: Record<string, any>, key: string) {
  return data?.[key] ?? (arboraDefaultData as Record<string, any>)[key] ?? "";
}
...
function Header({
  ...
  const nav = arboraPages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
  ...
  return (
    <header
      ...
      style={{ background: "#eef2eaee", borderColor: "rgba(28,38,24,0.15)", backdropFilter: "blur(14px)" }}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="flex items-center gap-3 text-start">
          <span
            ...
            style={{ background: "#4f6b45", color: "#eef2ea" }}
          >
            {v(data, "logoText")}
          </span>
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {nav.map(([id, label]) => (
            <button
              ...
              style={{ color: currentPage === id ? "#1c2618" : "#6b7564", opacity: currentPage === id ? 1 : 0.85 }}
            >
              {label}
            </button>
          ))}
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="arbora-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1c2618" }} placeholder={tx("ספרו בקצרה")}  name="message" data-bizuply-form-field-id="message"></textarea>
      <button type="submit" className="px-6 py-4 text-sm font-bold" style={{ background: "#4f6b45", color: "#0a0908" }}>
        {v(data, "cta")}
      </button>
    </form>
  );
...
function Hero({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  const layout: string = "editorial";

  if (layout === "type") {
    return (
      <section className="relative isolate min-h-[88vh] overflow-hidden">
        <img src={v(data, "heroImage")} alt="" className="absolute inset-0 h-full w-full object-cover" />
        <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, #eef2eaf0 0%, #eef2ea88 55%, transparent 100%)" }} />
        <div className="relative z-10 mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-5 pb-16 pt-28 lg:px-8">
          <p className="text-xs font-semibold tracking-[0.28em]" style={{ color: "#4f6b45" }}>{v(data, "heroEyebrow")}</p>
          <h1 className="tpl-display mt-4 max-w-4xl text-6xl font-extrabold leading-[0.95] md:text-8xl">{v(data, "heroTitle")}</h1>
          <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#6b7564" }}>{v(data, "heroSubtitle")}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#4f6b45", color: "#12180F" }}>{v(data, "heroPrimary")}</button>
            <button type="button" onClick={() => goTo("projects")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(28,38,24,0.15)" }}>{v(data, "heroSecondary")}</button>
          </div>
        </div>
      </section>
    );
  ...
  if (layout === "poster") {
    return (
      <section className="mx-auto grid max-w-7xl gap-0 px-5 py-10 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-16">
        <div className="flex flex-col justify-between border p-8 lg:min-h-[78vh] lg:p-12" style={{ borderColor: "rgba(28,38,24,0.15)", background: "#e2e8db" }}>
          <div>
            <p className="text-xs font-semibold tracking-[0.28em]" style={{ color: "#4f6b45" }}>{v(data, "heroEyebrow")}</p>
            <h1 className="tpl-display mt-8 text-6xl font-bold leading-[0.95] md:text-7xl">{v(data, "heroTitle")}</h1>
          </div>
          <div>
            <p className="max-w-md text-lg leading-8" style={{ color: "#6b7564" }}>{v(data, "heroSubtitle")}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#4f6b45", color: "#12180F" }}>{v(data, "heroPrimary")}</button>
              <button type="button" onClick={() => goTo("projects")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(28,38,24,0.15)" }}>{v(data, "heroSecondary")}</button>
            </div>
      ...
      <section className="relative isolate overflow-hidden">
        <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.05fr_0.95fr]">
          <div className="flex flex-col justify-center px-5 py-16 lg:px-8 lg:py-24">
            <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#4f6b45" }}>{v(data, "heroEyebrow")}</p>
            <h1 className="tpl-display mt-5 text-5xl font-bold leading-[1.02] md:text-7xl">{v(data, "heroTitle")}</h1>
            <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#6b7564" }}>{v(data, "heroSubtitle")}</p>
            <div className="mt-9 flex flex-wrap gap-3">
              <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#4f6b45", color: "#eef2ea" }}>{v(data, "heroPrimary")}</button>
              <button type="button" onClick={() => goTo("projects")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(28,38,24,0.15)" }}>{v(data, "heroSecondary")}</button>
            </div>
          </div>
  ...
  return (
    <section className="relative isolate min-h-[86vh] overflow-hidden">
      <img src={v(data, "heroImage")} alt="" className="absolute inset-0 h-full w-full object-cover" />
      <div className="absolute inset-0" style={{ background: "linear-gradient(105deg, #eef2eaf2 0%, #eef2ea99 48%, transparent 78%)" }} />
      <div className="relative z-10 mx-auto flex min-h-[86vh] max-w-7xl items-end px-5 pb-16 pt-28 lg:px-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-6 max-w-lg text-lg leading-8" style={{ color: "#6b7564" }}>{v(data, "heroSubtitle")}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#4f6b45", color: "#12180F" }}>{v(data, "heroPrimary")}</button>
            <button type="button" onClick={() => goTo("projects")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(28,38,24,0.15)" }}>{v(data, "heroSecondary")}</button>
          </div>
    ...
    <section className="border-t" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className={cx("mx-auto grid max-w-7xl lg:grid-cols-2", reverse && "lg:[&>*:first-child]:order-2")}>
        <div className="flex flex-col justify-center px-5 py-16 lg:px-12 lg:py-24">
          ...
          <p className="mt-6 max-w-md text-base leading-8" style={{ color: "#6b7564" }}>{tx(text)}</p>
        </div>
        ...
        <div className="mt-12 divide-y" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
          {items.map(([title, meta, text]) => (
            <div key={title} className="grid gap-3 border-t py-8 md:grid-cols-[0.9fr_0.4fr_1.2fr] md:items-baseline" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
              ...
              <p className="text-sm font-semibold" style={{ color: "#4f6b45" }}>{meta}</p>
              <p className="text-base leading-7" style={{ color: "#6b7564" }}>{tx(text)}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="border-t" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-12 lg:py-24">
          ...
          <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#6b7564" }}>{v(data, "aboutText")}</p>
        </div>
    ...
    <section className="border-t px-5 py-20 lg:px-8 lg:py-28" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1fr]">
        <div>
          ...
          <p className="mt-5 max-w-md text-base leading-8" style={{ color: "#6b7564" }}>{v(data, "contactText")}</p>
          <div className="mt-10 space-y-4 text-sm">
            <p><span style={{ color: "#6b7564" }}>{tx("טלפון ·")}</span>{v(data, "phone")}</p>
            <p><span style={{ color: "#6b7564" }}>{tx("אימייל ·")}</span>{v(data, "email")}</p>
            <p><span style={{ color: "#6b7564" }}>{tx("כתובת ·")}</span>{v(data, "address")}</p>
          </div>
        </div>
...
function Footer({ data }: { data: Record<string, any> }) {
  return (
    <footer className="border-t px-5 py-8 lg:px-8" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-3 text-sm md:flex-row md:items-center" style={{ color: "#6b7564" }}>
        <p>© {new Date().getFullYear()} {v(data, "brandName")}</p>
        <p>{tx("תבנית Arbora · Bizuply Studio")}</p>
      </div>
    </footer>
  );
}
  ...
  return (
    <>
      <section className="border-b px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
        <div className="mx-auto max-w-7xl">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#4f6b45" }}>{v(data, "brandName")}</p>
...
export default function ArboraPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const merged = useMemo(() => ({ ...arboraDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div
      ...
      style={{ background: "#eef2ea", color: "#1c2618" }}
    >
