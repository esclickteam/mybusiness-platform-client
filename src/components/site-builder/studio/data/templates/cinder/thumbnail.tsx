...
export default function CinderThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden" style={{ background: "#1a120e", color: "#f6efe6" }}>
      ...
      <div className="relative z-10 flex h-full min-h-[260px] flex-col justify-end p-5">
        <p className="text-[10px] font-semibold tracking-[0.22em]" style={{ color: "#e08a3c" }}>{tx("בית קלייה · יפו")}</p>
        <h3 className="mt-2 text-3xl font-bold leading-none" style={{ fontFamily: '"Rubik", serif' }}>Cinder</h3>
        <p className="mt-2 max-w-[220px] text-xs leading-5 opacity-80">{tx("סינדר היא בית קלייה ושולחן טעימות. פולים טריים, …")}</p>
      </div>
    </div>
  );
}
