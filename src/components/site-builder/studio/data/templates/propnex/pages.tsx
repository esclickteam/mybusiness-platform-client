...
function Header({ data, currentPage, goTo, onCta }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void; onCta: () => void }) {
  ...
  const nav = propnexPages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
  return (
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="propnex-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(17,24,39,0.1)", color: "#111827" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(17,24,39,0.1)", color: "#111827" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(17,24,39,0.1)", color: "#111827" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(17,24,39,0.1)", color: "#111827" }} placeholder={tx("מה אתם מחפשים?")}  name="message" data-bizuply-form-field-id="message"></textarea>
      <button type="submit" className="px-6 py-4 text-sm font-bold" style={{ background: "#2563eb", color: "#ffffff" }}>{v(data, "cta")}</button>
    </form>
  );
}
...
function BentoMarketPulse({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(17,24,39,0.1)", background: "#ffffff" }}>
      <h2 className="tpl-display mx-auto max-w-7xl text-2xl sm:text-4xl font-bold">{tx("תובנות Bento")}</h2>
      ...
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-4">
        {steps.map(([t,d],i) => (
          <div key={t} className="tpl-climb border-t pt-4" style={{ borderColor: "#2563eb", animationDelay: `${i*0.1}s` }}>
            ...
            <h3 className="mt-2 font-bold">{t}</h3><p className="mt-1 text-sm" style={{ color: "#6b7280" }}>{tx(d)}</p>
          </div>
        ))}
      </div>
    </section>
    ...
    <section className="border-t" style={{ borderColor: "rgba(17,24,39,0.1)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-8 lg:py-20">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#2563eb" }}>{tx("אודות")}</p>
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(17,24,39,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        <div>
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#2563eb" }}>{tx("יצירת קשר")}</p>
...
export default function PropnexPages({
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    pageContent["neighborhoods"] = (
      <InnerPage data={merged} title={tx("שכונות")} onCta={() => goTo("contact")}>
        <>
