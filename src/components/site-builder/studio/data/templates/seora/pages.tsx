...
const allowedPages = seoraPages.map((p) => p.id);
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 rounded-b-[2rem] border-b border-white/15 bg-[var(--dark)]/90 text-white backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-full bg-[var(--p)] text-sm font-black text-white">SE</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "seora:PageHero:3:data dashboard serp";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "seora:Hero:3:data dashboard serp";
  const stats = [
...
function About({ data }: { data: Record<string, any> }) {
  void "seora:About:3:data dashboard serp";
  return (
        ...
        <Reveal variant="right" className="">
          <div className="relative overflow-hidden rounded-[3rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">DATA DASHBOARD SERP</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "seora:Services:3:data dashboard serp";
  const services = [
...
function Cases({ data }: { data: Record<string, any> }) {
  void "seora:Cases:3:data dashboard serp";
  const items = [
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[1fr_1fr_0.8fr]">
          {items.map(([title, text, image], i) => (
            <Reveal key={title} delayMs={i * 110} variant="up">
              <article className="ag-card relative min-h-[520px] overflow-hidden rounded-t-[5rem] rounded-b-3xl border border-current/10 bg-[var(--surface)] text-start">
...
function Team({ data }: { data: Record<string, any> }) {
  void "seora:Team:3:data dashboard serp";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "seora:Gallery:3:data dashboard serp";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "seora:WhyUs:3:data dashboard serp";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "seora:Method:3:data dashboard serp";
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {steps.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 100} variant="scale">
              <article className="ag-card rounded-[2rem] border border-current/10 bg-[var(--surface)]/80 p-6 text-start ">
...
function Stats({ data }: { data: Record<string, any> }) {
  void "seora:Stats:3:data dashboard serp";
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "seora:Outcomes:3:data dashboard serp";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "seora:Insights:3:data dashboard serp";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "seora:Pricing:3:data dashboard serp";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "seora:Faq:3:data dashboard serp";
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "seora:CTABand:3:data dashboard serp";
  return (
...
function Contact({ data }: { data: Record<string, any> }) {
  void "seora:Contact:3:data dashboard serp";
  return (
    <section className="bg-[var(--dark)] text-white px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[2rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="seora-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "seora:Footer:3:data dashboard serp";
  const links = seoraPages.filter((p) => p.id !== "home");
  return (
...
export default function SeoraPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...seoraDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="seora" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: seoraEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={seoraPages.map((p) => ({
          id: p.id,
