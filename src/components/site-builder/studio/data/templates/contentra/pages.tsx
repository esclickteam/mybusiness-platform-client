...
const allowedPages = contentraPages.map((p) => p.id);
...
function getValue(data: Record<string, any>, key: string) {
  return data?.[key] ?? (contentraDefaultData as Record<string, any>)[key] ?? "";
}

function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  void "contentra:Header:4:editorial magazine";
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 border-b border-white/10 bg-[var(--bg)]/80 text-[var(--text)] shadow-[0_12px_40px_rgba(0,0,0,0.18)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-2xl bg-[var(--p)] text-sm font-black text-white">CO</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "contentra:PageHero:4:editorial magazine";
  ...
  return (
    <section className="border-y border-current/10 bg-[var(--surface)] px-5 py-20 lg:px-8">
      ...
      <Reveal variant="right" className="mx-auto max-w-7xl text-start">
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "contentra:Hero:4:editorial magazine";
  const stats = [
...
function About({ data }: { data: Record<string, any> }) {
  void "contentra:About:4:editorial magazine";
  return (
        ...
        <Reveal variant="scale" className="order-2 lg:order-1">
          <div className="relative overflow-hidden rounded-t-[5rem] rounded-b-3xl border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">EDITORIAL MAGAZINE</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "contentra:Services:4:editorial magazine";
  const services = [
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-2">
          {services.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 90} variant="up">
              <article className="ag-card group min-h-[260px] rounded-[2rem] border border-current/10 bg-[var(--bg)]/60 p-6 text-start ">
...
function Cases({ data }: { data: Record<string, any> }) {
  void "contentra:Cases:4:editorial magazine";
  const items = [
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[0.7fr_1.3fr]">
          {items.map(([title, text, image], i) => (
            <Reveal key={title} delayMs={i * 110} variant="scale">
              <article className="ag-card relative min-h-[430px] overflow-hidden rounded-[3rem] border border-current/10 bg-[var(--surface)] text-start">
...
function Team({ data }: { data: Record<string, any> }) {
  void "contentra:Team:4:editorial magazine";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "contentra:Gallery:4:editorial magazine";
  const images = [
        ...
        <div className="grid gap-4 md:grid-cols-[1.3fr_0.7fr_1fr]">
          {images.map((image, i) => (
            <Reveal key={image} delayMs={i * 80} variant="scale" className="">
              <div className="ag-card h-[360px] overflow-hidden rounded-[2.5rem] border border-current/10 bg-[var(--surface)] p-2">
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "contentra:WhyUs:4:editorial magazine";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "contentra:Method:4:editorial magazine";
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {steps.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 100} variant="up">
              <article className="ag-card rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--surface)]/80 p-6 text-start text-[var(--text)]">
...
function Stats({ data }: { data: Record<string, any> }) {
  void "contentra:Stats:4:editorial magazine";
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "contentra:Outcomes:4:editorial magazine";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "contentra:Insights:4:editorial magazine";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "contentra:Pricing:4:editorial magazine";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "contentra:Faq:4:editorial magazine";
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "contentra:CTABand:4:editorial magazine";
  return (
...
function Contact({ data }: { data: Record<string, any> }) {
  void "contentra:Contact:4:editorial magazine";
  return (
    <section className="bg-[var(--surface)] px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="contentra-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "contentra:Footer:4:editorial magazine";
  const links = contentraPages.filter((p) => p.id !== "home");
  return (
...
export default function ContentraPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...contentraDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="contentra" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: contentraEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={contentraPages.map((p) => ({
          id: p.id,
