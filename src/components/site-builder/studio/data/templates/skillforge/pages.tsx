...
const allowedPages = skillforgePages.map((page) => page.id);

type PageEntry = (typeof skillforgePages)[number];
...
function Header({ data, currentPage, goTo, openModal }: { data: Record<string, any>; currentPage: string; goTo: (pageId: string) => void; openModal: () => void }) {
  return (
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 border-b border-[var(--a)]/30 bg-[var(--dark)]/95 text-white backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="t-display text-xl font-black uppercase tracking-wider text-[var(--a)]">{getValue(data,"logoText")} · {getValue(data,"brandName")}</button>
        <nav className="hidden items-center gap-4 text-xs font-black uppercase tracking-widest text-white/55 lg:flex">
          {skillforgePages.map((page) => (
        ...
        <button type="button" onClick={openModal} className="bg-[var(--a)] px-5 py-2.5 text-sm font-black uppercase text-[var(--dark)]">{getValue(data,"heroPrimaryButton")}</button>
      </div>
    </header>
  );
}
...
function Hero({ data, openModal }: { data: Record<string, any>; openModal: () => void }) {
  return (
    <section data-template-section-type="hero" className="relative min-h-[90svh] overflow-hidden bg-[var(--bg)] px-5 py-20 lg:px-8">
      <div className="absolute inset-0 opacity-25"><SafeImg src={getValue(data,"heroImage")} alt="" className="t-ken h-full w-full object-cover grayscale" /></div>
...
function Curriculum({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="curriculum" className="border-y-2 border-[var(--p)] bg-[var(--surface)] px-5 py-20 lg:px-8">
      <Reveal><h2 className="t-display text-2xl sm:text-4xl font-bold uppercase">{getValue(data,"sectionThreeTitle")}</h2></Reveal>
...
function Instructors({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="instructors" className="px-5 py-12 md:py-24 lg:px-8">
      <Reveal><h2 className="t-display text-2xl sm:text-4xl font-bold uppercase text-[var(--p)]">{getValue(data,"sectionFourTitle")}</h2></Reveal>
        ...
        {[["רן כהן","COACH"],["דנה לוי","LEAD"],["עידו שמש","PRO"]].map(([n,r],i) => (
          <Reveal key={n} delayMs={i*90} className="t-hover border-2 border-[var(--p)] bg-[var(--surface)] p-4">
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 overflow-hidden border border-[var(--p)]"><SafeImg src={getValue(data,"sectionImage")} alt="" className="h-full w-full object-cover grayscale" /></div>
              <div>
...
function Stats({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="stats" className="bg-black px-5 py-12 lg:px-8">
      <div className="mx-auto grid max-w-5xl grid-cols-1 md:grid-cols-3 gap-3">
  ...
  return (
    <section data-template-section-type="faq" className="px-5 py-12 md:py-24 lg:px-8">
      <div className="mx-auto max-w-3xl">
        <Reveal><h2 className="t-display mb-8 text-2xl sm:text-4xl font-bold uppercase text-[var(--p)]">{getValue(data,"sectionSevenTitle")}</h2></Reveal>
...
function PageHero({ data, page, goTo }: PageProps & { page: PageEntry }) {
  return (
    <section data-template-section-type="pageHero" className="relative overflow-hidden px-5 py-14 md:py-28 lg:px-8 bg-[var(--dark)] text-white">
      <SafeImg src={getValue(data,"heroImage")} alt="" className="absolute inset-0 h-full w-full object-cover contrast-125 grayscale" />
      ...
      <Reveal className="relative z-10 mx-auto max-w-5xl text-center">
        ...
        <button type="button" onClick={() => goTo("contact")} className="mt-8 inline-flex px-6 py-3 text-sm font-bold bg-[var(--a)] text-[var(--dark)]">{getValue(data,"ctaBandButton")}</button>
      </Reveal>
    </section>
  );
}
...
function About({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="about" className="px-5 py-12 md:py-24 lg:px-8 bg-[var(--dark)] text-white">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
        <Reveal className="relative min-h-[360px] overflow-hidden border-2 border-[var(--a)]/35 bg-[var(--surface)]">
          <SafeImg src={getValue(data,"sectionImage")} alt="" className="absolute inset-0 h-full w-full object-cover contrast-125 grayscale" />
          <div className="absolute inset-0 bg-gradient-to-t from-[var(--dark)]/80 to-transparent" />
          <span className="absolute bottom-6 right-6 rounded-full px-4 py-2 text-sm font-bold bg-[var(--a)] text-[var(--dark)]">מיומנויות</span>
        </Reveal>
        <Reveal variant="up" className="border-2 border-[var(--a)]/35 bg-[var(--surface)] p-8 lg:p-12">
...
function WhyUs({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="why" className="px-5 py-12 md:py-24 lg:px-8 bg-[var(--dark)] text-white">
      ...
      <div className="mt-12 mx-auto grid max-w-5xl gap-4 md:grid-cols-3">
...
function Method({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="method" className="px-5 py-12 md:py-24 lg:px-8 bg-[var(--dark)] text-white">
      <Reveal className="mx-auto max-w-3xl text-center">
...
function Gallery({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="gallery" className="px-5 py-12 md:py-24 lg:px-8 bg-[var(--dark)] text-white">
      <Reveal className="mx-auto max-w-3xl text-center">
      ...
      <div className="mx-auto mt-12 grid max-w-5xl grid-cols-2 gap-3 md:grid-cols-4">
        {galleryItems(data).map((image, i) => (
          <Reveal key={image} delayMs={i * 80} className="t-hover relative min-h-[260px] overflow-hidden border-2 border-[var(--a)]/35 bg-[var(--surface)]">
            <SafeImg src={image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-700 hover:scale-110 contrast-125 grayscale" />
            <span className="absolute bottom-4 right-4 px-3 py-1 text-xs font-bold bg-[var(--a)] text-[var(--dark)]">0{i + 1}</span>
          </Reveal>
        ))}
      </div>
    </section>
...
function Outcomes({ data }: { data: Record<string, any> }) {
  return (
    <section data-template-section-type="outcomes" className="px-5 py-20 lg:px-8 bg-[var(--dark)] text-white">
      <div className="mx-auto max-w-6xl border-2 border-[var(--a)]/35 bg-[var(--surface)] p-8 lg:p-12">
        <Reveal>
...
function Pricing({ data, goTo }: Pick<PageProps, "data" | "goTo">) {
  return (
    <section data-template-section-type="pricing" className="px-5 py-12 md:py-24 lg:px-8 bg-[var(--dark)] text-white">
      <Reveal className="mx-auto max-w-3xl text-center">
...
function Insights({ data, goTo }: Pick<PageProps, "data" | "goTo">) {
  return (
    <section data-template-section-type="insights" className="px-5 py-12 md:py-24 lg:px-8 bg-[var(--dark)] text-white">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.7fr_1.3fr]">
        <Reveal>
          ...
          <button type="button" onClick={() => goTo("faq")} className="mt-8 px-5 py-3 text-sm font-bold bg-[var(--a)] text-[var(--dark)]">{getValue(data,"navFaq")}</button>
        </Reveal>
...
function CTABand({ data, goTo }: Pick<PageProps, "data" | "goTo">) {
  return (
    <section data-template-section-type="cta" className="px-5 py-20 lg:px-8 bg-[var(--dark)] text-white">
      <Reveal className="mx-auto max-w-5xl border-2 border-[var(--a)]/35 bg-[var(--surface)] p-8 text-center lg:p-14">
        ...
        <button type="button" onClick={() => goTo("contact")} className="mt-8 px-8 py-4 text-sm font-bold bg-[var(--a)] text-[var(--dark)]">{getValue(data,"ctaBandButton")}</button>
      </Reveal>
    </section>
  );
}
  ...
  return (
    <section data-template-section-type="contact" className="px-5 py-12 md:py-24 lg:px-8">
      <div className="mx-auto max-w-xl border-2 border-[var(--p)] p-8">
        ...
        <form className="mt-6 grid gap-3" data-bizuply-block="lead-form" data-bizuply-form-id="skillforge-contact-1" data-bizuply-crm-lead="true" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אליכם בהקדם.">
          <input name="name" data-bizuply-form-field-id="name" autoComplete="name"  className="border border-[var(--muted)]/40 bg-transparent px-4 py-3 uppercase outline-none" placeholder="FULL NAME" />
          <input name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel"  className="border border-[var(--muted)]/40 bg-transparent px-4 py-3 uppercase outline-none" placeholder="PHONE" />
          <button type="submit" onClick={openModal} className="bg-[var(--p)] py-4 font-black uppercase text-[var(--dark)]">{getValue(data,"contactButton")}</button>
        </form>
      </div>
    </section>
  );
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-black/70 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md border border-[var(--p)]/30 bg-[var(--surface)] p-8">
        ...
        <form className="mt-6 grid gap-3" data-bizuply-block="lead-form" data-bizuply-form-id="skillforge-contact-2" data-bizuply-crm-lead="true" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אליכם בהקדם.">
...
export default function SkillforgePages(props: SkillforgePagesProps) {
  ...
  return (
    <div dir="rtl" data-template-id={mode === "preview" ? "skillforge-preview" : "skillforge"} className="min-h-screen w-full overflow-x-hidden">
      ...
      <VisualPageStack activePageId={currentPage} pages={skillforgePages.map((page) => ({ id: page.id, content: <>{(pageSectionOrder[page.id] ?? pageSectionOrder.home).map((sectionName, index) => <React.Fragment key={page.id + "-" + sectionName + "-" + index}>{renderSection(sectionName, page, { data: mergedData, openModal: () => setModalOpen(true), goTo })}</React.Fragment>)}</> }))} />
