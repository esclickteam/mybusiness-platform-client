...
function Marquee({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="overflow-hidden border-y border-black/10 bg-[var(--p)] py-4 text-white">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {items.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 90} variant="up">
              <article className={`ag-card min-h-[260px] border border-black/10 bg-[var(--surface)] p-7 text-right`}>
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {items.map(([name, role], i) => (
            <Reveal key={name} delayMs={i * 90} variant="scale">
              <article className={`ag-card border border-black/10 bg-[var(--bg)] p-8 text-center`}>
