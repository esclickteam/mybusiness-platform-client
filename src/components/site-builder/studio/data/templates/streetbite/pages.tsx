  ...
  return (
    <header data-template-section-type="header" data-section-kind="header" className="sticky top-0 z-50" style={{ background: "transparent" }}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => go("home")} className="-rotate-2 border-2 px-4 py-2 shadow-[4px_4px_0_0_#39d353]"
        ...
        <nav className="hidden items-center gap-3 lg:flex">
          {nav.map(([id, label]) => (
            <button key={id} type="button" onClick={() => go(id)} className="rotate-1 border px-3 py-1.5 text-xs font-bold"
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {[v(data, "gallery1Image"), v(data, "gallery2Image"), v(data, "gallery3Image")].map((src, i) => (
            <div key={i} className="journal-card-media tpl-float relative aspect-square overflow-hidden border" style={{ borderColor: "rgba(230,237,243,0.12)", animationDelay: `${i * 0.25}s` }}>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-12">
          <Reveal delayMs={0} variant="up" className="lg:col-span-12">
            <article className="overflow-hidden border rotate-[-1deg] group lg:grid lg:grid-cols-2" style={{ borderColor: "rgba(230,237,243,0.12)", background: "#161b22" }}>
          ...
          {rest.map((post, i) => (
            <Reveal key={post.title} delayMs={(i + 1) * 90} variant="up" className="lg:col-span-6">
              <article className="overflow-hidden border rotate-[-1deg] group h-full" style={{ borderColor: "rgba(230,237,243,0.12)", background: "#161b22" }}>
      ...
      <div className="mx-auto max-w-md">
        <div className="rounded-2xl border p-4" style={{ borderColor: "rgba(230,237,243,0.12)", background: "#0d1117" }}>
          ...
          <form className="grid gap-2" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="streetbite-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
            <input className="w-full rounded-full border bg-transparent px-4 py-3 text-right text-sm outline-none" style={{ borderColor: "rgba(230,237,243,0.12)" }} placeholder={tx("הקלידו הודעה...")} name="message" data-bizuply-form-field-id="message" />
