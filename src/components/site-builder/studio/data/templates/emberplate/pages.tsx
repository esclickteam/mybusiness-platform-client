    ...
    <header data-template-section-type="header" data-section-kind="header" className="sticky top-0 z-50 border-b"
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3.5 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => { setOpen(false); onCta(); }} className="hidden px-5 py-2.5 text-sm font-bold sm:inline-flex"
...
function SectionKicker({ label }: { label: string }) {
  return <p className="text-xs font-bold tracking-[0.28em]" style={{ color: "#e85d04" }}>{label}</p>;
}
...
function PageHero({ data, title, pageId }: { data: Record<string, any>; title: string; pageId: string }) {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b px-5 py-20 lg:px-8 lg:py-24" style={{ borderColor: "rgba(246,235,224,0.12)" }}>
      <img src={image} alt="" className="tpl-ken absolute inset-0 h-full w-full object-cover opacity-30" />
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, #140c08f5, #1f1410cc)" }} />
      <div className="relative z-10 mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8" style={{ color: "#b89a82" }}>{v(data, "pageHeroText")}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {[v(data, "gallery1Image"), v(data, "gallery2Image"), v(data, "gallery3Image")].map((src, i) => (
            <div key={i} className="journal-card-media tpl-float relative aspect-square overflow-hidden border" style={{ borderColor: "rgba(246,235,224,0.12)", animationDelay: `${i * 0.25}s` }}>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {reasons.map((reason, i) => (
            <Reveal key={reason.title} delayMs={i * 90} variant="up">
              <article className="tpl-sweep h-full border p-6 " style={{ borderColor: "rgba(246,235,224,0.12)", background: i % 2 ? "#140c08" : "#0a0604" }}>
                <span className="tpl-display text-2xl md:text-5xl font-bold" style={{ color: "#e85d04" }}>0{i + 1}</span>
                <h3 className="mt-5 text-xl font-bold">{reason.title}</h3>
                <p className="mt-3 text-sm leading-7" style={{ color: "#b89a82" }}>{reason.text}</p>
              </article>
            </Reveal>
          ))}
        </div>
...
function MenuShowcase({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(246,235,224,0.12)" }}>
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-md text-sm leading-7" style={{ color: "#b89a82" }}>{v(data, "menuShowcaseText")}</p>
        </div>
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {dishes.map((dish, i) => (
            <Reveal key={dish.title} delayMs={i * 100} variant="scale">
              <article className="group overflow-hidden border " style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
                <img src={dish.img} alt="" className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-105" />
                <div className="p-5">
                  <p className="text-xs font-bold tracking-[0.18em]" style={{ color: "#e85d04" }}>{dish.meta}</p>
                  <h3 className="tpl-display mt-2 text-2xl font-bold">{dish.title}</h3>
                  <p className="mt-3 text-sm leading-7" style={{ color: "#b89a82" }}>{dish.text}</p>
                </div>
              </article>
            </Reveal>
          ))}
        ...
        <div className="mt-10 grid gap-3 md:grid-cols-4">
          {shots.map((shot, i) => (
            <Reveal key={shot.label} delayMs={i * 80} variant={i % 2 ? "up" : "scale"}>
              <figure className={`relative overflow-hidden border ${i === 0 ? "md:row-span-2" : ""} `} style={{ borderColor: "rgba(246,235,224,0.12)" }}>
                <img src={shot.src} alt="" className={i === 0 ? "aspect-[4/5] h-full w-full object-cover" : "aspect-square w-full object-cover"} />
                <figcaption className="absolute inset-x-3 bottom-3 px-3 py-2 text-sm font-bold" style={{ background: "#140c08dd", color: "#f6ebe0" }}>{shot.label}</figcaption>
              </figure>
            </Reveal>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          <div>
            ...
            <p className="mt-5 text-sm leading-7" style={{ color: "#b89a82" }}>{v(data, "teamText")}</p>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {people.map((person, i) => (
              <Reveal key={person.name} delayMs={i * 90} variant="up">
                <article className="border p-3 " style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
                  <img src={person.img} alt="" className="aspect-[4/5] w-full object-cover" />
                  <h3 className="mt-4 text-lg font-bold">{person.name}</h3>
                  <p className="text-sm" style={{ color: "#e85d04" }}>{person.role}</p>
                </article>
              </Reveal>
            ))}
          </div>
...
function KitchenMethod({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(246,235,224,0.12)", background: "#0a0604" }}>
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-4">
          {steps.map((step, i) => (
            <Reveal key={step.title} delayMs={i * 80} variant="right">
              <article className="relative min-h-48 border p-5 " style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
                <span className="tpl-display text-2xl md:text-5xl font-bold opacity-30" style={{ color: "#e85d04" }}>{i + 1}</span>
                <h3 className="mt-6 text-lg font-bold">{step.title}</h3>
                <p className="mt-3 text-sm leading-7" style={{ color: "#b89a82" }}>{step.text}</p>
              </article>
            </Reveal>
          ))}
        </div>
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-3">
          {reviews.map((review, i) => (
            <Reveal key={review.name} delayMs={i * 90} variant="fade">
              <blockquote className="h-full border p-6 " style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
                <p className="text-lg leading-8">״{review.quote}״</p>
                <footer className="mt-5 text-sm font-bold" style={{ color: "#e85d04" }}>{review.name}</footer>
              </blockquote>
            </Reveal>
          ))}
        </div>
    ...
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_1fr] lg:items-center">
        <div>
          ...
          <p className="mt-5 text-lg leading-8" style={{ color: "#b89a82" }}>{v(data, "visitText")}</p>
          <p className="mt-6 text-sm font-bold">{v(data, "address")}</p>
        </div>
        <div className="grid gap-3">
          {hours.map((hour, i) => (
            <Reveal key={hour.day} delayMs={i * 80} variant="left">
              <div className="flex items-center justify-between border px-5 py-4 " style={{ borderColor: "rgba(246,235,224,0.12)", background: "#140c08" }}>
                <span className="font-bold">{hour.day}</span>
                <span style={{ color: "#e85d04" }}>{hour.time}</span>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-12">
          <Reveal delayMs={0} variant="up" className="lg:col-span-12">
            <article className="overflow-hidden border group lg:grid lg:grid-cols-2" style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
          ...
          {rest.map((post, i) => (
            <Reveal key={post.title} delayMs={(i + 1) * 90} variant="up" className="lg:col-span-6">
              <article className="overflow-hidden border group h-full" style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
...
function CTABand({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(246,235,224,0.12)", background: "linear-gradient(135deg, #e85d0433, #1f1410)" }}>
      <div className="mx-auto flex max-w-7xl flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl leading-7" style={{ color: "#b89a82" }}>{v(data, "ctaBandText")}</p>
        </div>
        <button type="button" onClick={onCta} className="tpl-sweep px-7 py-4 text-sm font-bold" style={{ background: "#e85d04", color: "#140c08" }}>{v(data, "cta")}</button>
      </div>
    </section>
  );
}
    ...
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(246,235,224,0.12)", background: "#1f1410" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        ...
        <form className="tpl-ember-pulse grid gap-3 border p-6" style={{ borderColor: "#e85d04" }} data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="emberplate-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
          <input className="w-full border bg-transparent px-4 py-3.5 text-right outline-none" style={{ borderColor: "rgba(246,235,224,0.12)", color: "#f6ebe0" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
          <input className="w-full border bg-transparent px-4 py-3.5 text-right outline-none" style={{ borderColor: "rgba(246,235,224,0.12)", color: "#f6ebe0" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
          <input className="w-full border bg-transparent px-4 py-3.5 text-right outline-none" style={{ borderColor: "rgba(246,235,224,0.12)", color: "#f6ebe0" }} placeholder={tx("תאריך")} name="date" data-bizuply-form-field-id="date" />
