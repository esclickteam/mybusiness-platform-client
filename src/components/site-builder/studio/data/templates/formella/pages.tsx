...
function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  return (
    <header data-visual-flow-lock="true" data-template-section-type="header" data-section-kind="header" className="sticky top-0 z-50 border-b border-[var(--p)]/30 bg-[var(--bg)] text-[var(--text)]">
...
function HomePage({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  return (
    <>
      ...
      <footer data-template-section-type="footer" data-section-kind="footer" className="beauty-sculptInk-footer-sculpt border-t border-[var(--p)]/30 px-5 py-10 lg:px-8"><div className="mx-auto flex max-w-7xl items-center gap-4"><span className="bg-[var(--p)] px-3 py-2 font-black text-[var(--dark)]">{getValue(data,"logoText")}</span><span className="font-bold">{getValue(data,"brandName")}</span><span className="mr-auto text-sm text-[var(--muted)]">{getValue(data,"email")}</span></div></footer>
    </>
  );
}
