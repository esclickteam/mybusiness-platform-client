...
function Header({ data, currentPage, goTo, onCta }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void; onCta: () => void }) {
  ...
  const nav = openhausPages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
  return (
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="openhaus-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(41,37,36,0.1)", color: "#292524" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(41,37,36,0.1)", color: "#292524" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(41,37,36,0.1)", color: "#292524" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(41,37,36,0.1)", color: "#292524" }} placeholder={tx("מה אתם מחפשים?")}  name="message" data-bizuply-form-field-id="message"></textarea>
      <button type="submit" className="px-6 py-4 text-sm font-bold" style={{ background: "#ea580c", color: "#ffffff" }}>{v(data, "cta")}</button>
    </form>
  );
}
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 md:grid-cols-3 gap-4">
        {stats.map(([vk,lk],i) => (
          <div key={lk} className="tpl-climb border p-5 text-center" style={{ borderColor: "rgba(41,37,36,0.1)", animationDelay: `${i*0.1}s` }}>
            <div className="tpl-display text-2xl sm:text-4xl font-bold" style={{ color: "#ea580c" }}>{v(data,vk)}</div>
...
function PanelMarketPulse({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(41,37,36,0.1)", background: "#ffffff" }}>
      <h2 className="tpl-display mx-auto max-w-7xl text-2xl sm:text-4xl font-bold">{tx("תובנות Panel")}</h2>
    ...
    <section className="border-t" style={{ borderColor: "rgba(41,37,36,0.1)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-8 lg:py-20">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#ea580c" }}>{tx("אודות")}</p>
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(41,37,36,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        <div>
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#ea580c" }}>{tx("יצירת קשר")}</p>
...
export default function OpenhausPages({
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    pageContent["openhouses"] = (
      <InnerPage data={merged} title={tx("בתים פתוחים")} onCta={() => goTo("contact")}>
        <>
    ...
    pageContent["gallery"] = (
      <InnerPage data={merged} title={tx("גלריה")} onCta={() => goTo("contact")}>
        <>
          <MasonryGallery data={merged} />
      <PanelGalleryWall data={merged} />
