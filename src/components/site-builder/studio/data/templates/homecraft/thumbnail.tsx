...
export default function HomecraftThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative flex h-full min-h-[260px] w-full flex-col justify-between overflow-hidden p-5 text-right" style={{ background: "#FFFBF5", color: "#3F2A14", fontFamily: "Heebo, sans-serif" }}>
      ...
      <div className="relative">
        <div className="inline-flex px-2 py-1 text-[10px] font-black uppercase tracking-[0.2em]" style={{ background: "#B45309", color: "#FFFBEB" }}>{tx("בית ועיצוב")}</div>
        <h3 className="mt-4 text-3xl font-black leading-none" style={{ fontFamily: "Libre Baskerville, serif" }}>Homecraft</h3>
        <p className="mt-2 text-xs font-semibold opacity-70">{tx("חללים שמרגישים בית")}</p>
      </div>
