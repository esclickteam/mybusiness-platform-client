...
export default function NumerisThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-4" style={{ background: "#F3F6F4", color: "#143028", fontFamily: "\"Figtree\", sans-serif" }}>
      ...
      <div className="relative z-10 mt-5 grid grid-cols-[1fr_.78fr] gap-3">
        <div className="border bg-white p-4" style={{ borderColor: "rgba(15,110,86,.2)", borderRadius: 0 }}>
          <h3 className="text-2xl font-bold leading-[1.02] tracking-[-0.05em]" style={{ fontFamily: "\"Literata\", serif" }}>{tx("מספרים מסודרים.")}<br />{tx("החלטות שקטות.")}</h3>
          <div className="mt-5 space-y-2">
            {[tx("שירות"), tx("תיאור"), tx("מחיר")].map((item) => (
