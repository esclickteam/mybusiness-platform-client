  ...
  return (
    <header data-template-section-type="header" data-section-kind="header" className="sticky top-0 z-50 px-4 pt-3">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 border-2 px-5 py-3 lg:px-8"
        ...
        <button type="button" onClick={() => go("home")} className="tpl-display text-xl font-bold">{v(data, "brandName")}</button>
        <nav className="hidden items-center gap-5 lg:flex">
          {nav.map(([id, label]) => (
            <button key={id} type="button" onClick={() => go(id)} className="border-b-2 text-sm font-semibold"
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {[v(data, "gallery1Image"), v(data, "gallery2Image"), v(data, "gallery3Image")].map((src, i) => (
            <div key={i} className="journal-card-media tpl-float relative aspect-square overflow-hidden border" style={{ borderColor: "rgba(255,241,224,0.14)", animationDelay: `${i * 0.25}s` }}>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-12">
          <Reveal delayMs={0} variant="up" className="lg:col-span-12">
            <article className="overflow-hidden border rounded-[2rem] group lg:grid lg:grid-cols-2" style={{ borderColor: "rgba(255,241,224,0.14)", background: "#2a1810" }}>
          ...
          {rest.map((post, i) => (
            <Reveal key={post.title} delayMs={(i + 1) * 90} variant="up" className="lg:col-span-6">
              <article className="overflow-hidden border rounded-[2rem] group h-full" style={{ borderColor: "rgba(255,241,224,0.14)", background: "#2a1810" }}>
