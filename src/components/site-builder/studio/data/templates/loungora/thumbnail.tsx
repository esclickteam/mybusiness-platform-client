...
export default function LoungoraThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative flex h-full min-h-[260px] w-full flex-col justify-between overflow-hidden p-5 text-right" style={{ background: "#FAF5FF", color: "#4C1D95", fontFamily: "Heebo, sans-serif" }}>
      ...
      <div className="relative">
        <div className="inline-flex px-2 py-1 text-[10px] font-black uppercase tracking-[0.2em]" style={{ background: "#5B21B6", color: "#F5F3FF" }}>{tx("הלבשת בית")}</div>
        <h3 className="mt-4 text-3xl font-black leading-none" style={{ fontFamily: "Literata, serif" }}>Loungora</h3>
        <p className="mt-2 text-xs font-semibold opacity-70">{tx("נוחות שנראית כמו סטייל.")}</p>
      </div>
