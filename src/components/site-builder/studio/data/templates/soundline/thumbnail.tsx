...
export default function SoundlineThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-5" style={{ background: "#060609", color: "#F7F7FB" }}>
      ...
      <div className="relative">
        <div className="text-xs font-black uppercase tracking-[0.28em]" style={{ color: "#FF4D6D" }}>{tx("בית ספר למוזיקה")}</div>
        ...
        <p className="mt-5 max-w-[170px] text-sm font-bold leading-5 text-white/70">{tx("ניאון, במה, מורים וגלריית סטריפים")}</p>
      </div>
    </div>
  );
}
