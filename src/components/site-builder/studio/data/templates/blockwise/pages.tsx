...
function Header({ data, currentPage, goTo, onCta }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void; onCta: () => void }) {
  ...
  const nav = blockwisePages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
  return (
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="blockwise-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(26,26,26,0.18)", color: "#1a1a1a" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(26,26,26,0.18)", color: "#1a1a1a" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(26,26,26,0.18)", color: "#1a1a1a" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(26,26,26,0.18)", color: "#1a1a1a" }} placeholder={tx("מה אתם מחפשים?")}  name="message" data-bizuply-form-field-id="message"></textarea>
...
function BlockMarketPulse({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(26,26,26,0.18)", background: "#f5f2ee" }}>
      <h2 className="tpl-display mx-auto max-w-7xl text-2xl sm:text-4xl font-bold">{tx("תובנות Block")}</h2>
      ...
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-4">
        {steps.map(([t,d],i) => (
          <div key={t} className="tpl-climb border-t pt-4" style={{ borderColor: "#e63946", animationDelay: `${i*0.1}s` }}>
            ...
            <h3 className="mt-2 font-bold">{t}</h3><p className="mt-1 text-sm" style={{ color: "#666666" }}>{tx(d)}</p>
          </div>
        ))}
      </div>
    </section>
    ...
    <section className="border-t" style={{ borderColor: "rgba(26,26,26,0.18)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-8 lg:py-20">
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#e63946" }}>{tx("אודות")}</p>
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(26,26,26,0.18)", background: "#f5f2ee" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        <div>
          <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#e63946" }}>{tx("יצירת קשר")}</p>
...
export default function BlockwisePages({
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    pageContent["index"] = (
      <InnerPage data={merged} title={tx("אינדקס")} onCta={() => goTo("contact")}>
        <>
