...
function Header({
  ...
  const nav = solennePages.map((p) => [p.id, v(data, `nav${p.id[0].toUpperCase()}${p.id.slice(1)}`) || tx(p.label)] as const);
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="solenne-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1d1a17" }} placeholder={tx("שם מלא")} name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1d1a17" }} placeholder={tx("טלפון")} name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
      <input className={field} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1d1a17" }} placeholder={tx("אימייל")} name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(28,38,24,0.15)", color: "#1d1a17" }} placeholder={tx("ספרו בקצרה")}  name="message" data-bizuply-form-field-id="message"></textarea>
      <button type="submit" className="px-6 py-4 text-sm font-bold" style={{ background: "#8b6f5c", color: "#0a0908" }}>
        {v(data, "cta")}
      </button>
    </form>
  );
    ...
    <section className="border-t" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className={cx("mx-auto grid max-w-7xl lg:grid-cols-2", reverse && "lg:[&>*:first-child]:order-2")}>
        <div className="flex flex-col justify-center px-5 py-16 lg:px-12 lg:py-24">
          ...
          <p className="mt-6 max-w-md text-base leading-8" style={{ color: "#7a7168" }}>{tx(text)}</p>
        </div>
        ...
        <div className="mt-12 divide-y" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
          {items.map(([title, meta, text]) => (
            <div key={title} className="grid gap-3 border-t py-8 md:grid-cols-[0.9fr_0.4fr_1.2fr] md:items-baseline" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
              ...
              <p className="text-base leading-7" style={{ color: "#7a7168" }}>{tx(text)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1fr]">
        <div>
          ...
          <div className="mt-10 space-y-4 text-sm">
            <p><span style={{ color: "#7a7168" }}>{tx("טלפון ·")}</span>{v(data, "phone")}</p>
            <p><span style={{ color: "#7a7168" }}>{tx("אימייל ·")}</span>{v(data, "email")}</p>
            <p><span style={{ color: "#7a7168" }}>{tx("כתובת ·")}</span>{v(data, "address")}</p>
          </div>
        </div>
  ...
  return (
    <footer className="border-t px-5 py-8 lg:px-8" style={{ borderColor: "rgba(28,38,24,0.15)" }}>
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-3 text-sm md:flex-row md:items-center" style={{ color: "#7a7168" }}>
        ...
        <p>{tx("תבנית Solenne · Bizuply Studio")}</p>
      </div>
    </footer>
  );
}
