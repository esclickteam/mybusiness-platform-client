...
export default function AurayogaThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-5" style={{ background: "#1C1526", color: "#F5F0FF" }}>
      ...
      <div className="relative">
        <div className="text-xs uppercase tracking-[0.28em]" style={{ color: "#A78BFA" }}>{tx("סטודיו יוגה")}</div>
        <h3 className="mt-10 text-5xl font-semibold leading-none" style={{ fontFamily: "serif", letterSpacing: "-0.06em" }}>AuraYoga</h3>
        <p className="mt-4 max-w-[170px] text-sm leading-5 text-white/70">{tx("תבנית רגועה עם קלפים מרובעים ותנועה רכה")}</p>
      </div>
    </div>
  );
}
