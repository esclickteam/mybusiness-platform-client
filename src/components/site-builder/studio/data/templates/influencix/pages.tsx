...
const allowedPages = influencixPages.map((p) => p.id);
...
function getValue(data: Record<string, any>, key: string) {
  return data?.[key] ?? (influencixDefaultData as Record<string, any>)[key] ?? "";
}

function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  void "influencix:Header:2:creator spotlight";
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "influencix:PageHero:2:creator spotlight";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "influencix:Hero:2:creator spotlight";
  const stats = [
...
function About({ data }: { data: Record<string, any> }) {
  void "influencix:About:2:creator spotlight";
  return (
        ...
        <Reveal variant="scale" className="order-2 lg:order-1">
          <div className="relative overflow-hidden rounded-[1rem_5rem_1rem_5rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">CREATOR SPOTLIGHT</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "influencix:Services:2:creator spotlight";
  const services = [
...
function Cases({ data }: { data: Record<string, any> }) {
  void "influencix:Cases:2:creator spotlight";
  const items = [
...
function Team({ data }: { data: Record<string, any> }) {
  void "influencix:Team:2:creator spotlight";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "influencix:Gallery:2:creator spotlight";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "influencix:WhyUs:2:creator spotlight";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "influencix:Method:2:creator spotlight";
...
function Stats({ data }: { data: Record<string, any> }) {
  void "influencix:Stats:2:creator spotlight";
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "influencix:Outcomes:2:creator spotlight";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "influencix:Insights:2:creator spotlight";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "influencix:Pricing:2:creator spotlight";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "influencix:Faq:2:creator spotlight";
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "influencix:CTABand:2:creator spotlight";
  return (
...
function Contact({ data }: { data: Record<string, any> }) {
  void "influencix:Contact:2:creator spotlight";
  return (
    <section className="bg-[var(--surface)] px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[1rem_3rem_1rem_3rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="influencix-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "influencix:Footer:2:creator spotlight";
  const links = influencixPages.filter((p) => p.id !== "home");
  return (
...
export default function InfluencixPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...influencixDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="influencix" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: influencixEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={influencixPages.map((p) => ({
          id: p.id,
