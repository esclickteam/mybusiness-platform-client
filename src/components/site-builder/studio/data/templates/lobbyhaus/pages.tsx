...
const allowedPages = lobbyhausPages.map((p) => p.id);
...
function getValue(data: Record<string, any>, key: string) {
  return data?.[key] ?? (lobbyhausDefaultData as Record<string, any>)[key] ?? "";
}

function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  void "lobbyhaus:Header:13:institutional formal";
  ...
  return (
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 border-b border-white/10 bg-[var(--bg)]/80 text-[var(--text)] shadow-[0_12px_40px_rgba(0,0,0,0.18)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-full bg-[var(--p)] text-sm font-black text-white">LO</span>
          <span>
      ...
      {open ? (
        <div className="border-t border-current/10 px-5 py-4 lg:hidden">
          <div className="grid gap-2">
            {nav.map(([id, label]) => (
              <button key={id} type="button" onClick={() => { goTo(id); setOpen(false); }} className="rounded-2xl bg-current/5 px-4 py-3 text-start text-sm font-bold">{label}</button>
            ))}
          </div>
        </div>
      ) : null}
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "lobbyhaus:PageHero:13:institutional formal";
  ...
  return (
    <section className="border-y border-current/10 bg-[var(--surface)] px-5 py-20 lg:px-8">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[var(--a)]/60" />
      <Reveal variant="up" className="mx-auto max-w-7xl text-start">
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "lobbyhaus:Hero:13:institutional formal";
  const stats = [
  ...
  return (
    <section data-template-section-type="hero" className="relative min-h-[90svh] overflow-hidden bg-[var(--bg)] px-5 py-20 lg:px-8">
      ...
      <div className="relative z-10 mx-auto grid min-h-[80svh] max-w-7xl items-end gap-10 lg:grid-cols-[1.2fr_0.8fr]">
        <Reveal variant="up" className="rounded-[2.5rem] border border-white/15 bg-[var(--surface)]/85 p-7 text-start backdrop-blur-xl md:p-10">
...
function About({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:About:13:institutional formal";
  return (
    <section className="px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.2fr_0.8fr] items-center">
        <Reveal variant="right" className="">
          <div className="relative overflow-hidden rounded-t-[5rem] rounded-b-3xl border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">INSTITUTIONAL FORMAL</p>
              <p className="ag-display mt-2 text-3xl font-black">{getValue(data, "statOne")}</p>
            </div>
          </div>
        </Reveal>
        <Reveal variant="up" className="text-start">
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "lobbyhaus:Services:13:institutional formal";
  const services = [
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-2">
          {services.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 90} variant="scale">
              <article className="ag-card group min-h-[260px] rounded-[2rem] border border-current/10 bg-[var(--bg)]/60 p-6 text-start backdrop-blur">
...
function Cases({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Cases:13:institutional formal";
  const items = [
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[0.7fr_1.3fr]">
          {items.map(([title, text, image], i) => (
            <Reveal key={title} delayMs={i * 110} variant="up">
              <article className="ag-card relative min-h-[430px] overflow-hidden rounded-t-[5rem] rounded-b-3xl border border-current/10 bg-[var(--surface)] text-start">
...
function Team({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Team:13:institutional formal";
  const people = [
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {people.map(([name, role, image], i) => (
            <Reveal key={name} delayMs={i * 100} variant="scale">
              <article className="ag-card overflow-hidden rounded-full border border-current/10 bg-[var(--surface)] text-start">
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Gallery:13:institutional formal";
  const images = [
    ...
    <section className="px-5 py-16 lg:px-8 lg:py-24">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-4 md:grid-cols-[1.3fr_0.7fr_1fr]">
          {images.map((image, i) => (
            <Reveal key={image} delayMs={i * 80} variant="scale" className="">
              <div className="ag-card h-[360px] overflow-hidden rounded-[1rem_4rem_1rem_4rem] border border-current/10 bg-[var(--surface)] p-2">
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "lobbyhaus:WhyUs:13:institutional formal";
  const items = [
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {items.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 90} variant="scale">
              <article className="ag-card rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--surface)] p-7 text-start">
                <div className="mb-7 h-2 w-12 rounded-full bg-[var(--a)]" />
...
function Method({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Method:13:institutional formal";
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {steps.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 100} variant="scale">
              <article className="ag-card rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--surface)]/80 p-6 text-start ">
...
function Stats({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Stats:13:institutional formal";
      ...
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-4">
        {stats.map(([value, label], i) => (
          <Reveal key={label} delayMs={i * 80} variant="scale">
            <div className="ag-card rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--surface)] p-7 text-start">
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "lobbyhaus:Outcomes:13:institutional formal";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "lobbyhaus:Insights:13:institutional formal";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Pricing:13:institutional formal";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Faq:13:institutional formal";
        ...
        <div className="mt-10 grid gap-3">
          {faqs.map(([q, a], i) => (
            <Reveal key={q} delayMs={i * 80} variant="up">
              <article className="ag-card rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--surface)] p-6 text-start">
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "lobbyhaus:CTABand:13:institutional formal";
  return (
    <section className="px-5 py-12 lg:px-8">
      <Reveal variant="scale" className="mx-auto max-w-7xl overflow-hidden rounded-t-[4rem] rounded-b-2xl bg-[var(--p)] p-8 text-start text-white md:p-12">
...
function Contact({ data }: { data: Record<string, any> }) {
  void "lobbyhaus:Contact:13:institutional formal";
  return (
    <section className="bg-[var(--dark)] text-white px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-t-[4rem] rounded-b-2xl border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="lobbyhaus-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
            <input className="rounded-2xl border border-current/10 bg-[var(--surface)] px-4 py-4 text-start outline-none" placeholder={tx("שם מלא")}  name="name" data-bizuply-form-field-id="name" type="text" autoComplete="name" />
            <div className="grid gap-3 md:grid-cols-2">
              <input className="rounded-2xl border border-current/10 bg-[var(--surface)] px-4 py-4 text-start outline-none" placeholder={tx("טלפון")}  name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel" />
              <input className="rounded-2xl border border-current/10 bg-[var(--surface)] px-4 py-4 text-start outline-none" placeholder={tx("חברה / תחום")}  name="company" data-bizuply-form-field-id="company" />
            </div>
            <textarea className="min-h-32 rounded-2xl border border-current/10 bg-[var(--surface)] px-4 py-4 text-start outline-none" placeholder={tx("במה נוכל לעזור?")}  name="message" data-bizuply-form-field-id="message"></textarea>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "lobbyhaus:Footer:13:institutional formal";
  const links = lobbyhausPages.filter((p) => p.id !== "home");
  return (
    <footer data-template-section-type="footer" className="bg-[var(--dark)] px-5 py-16 text-white lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-10 text-start lg:grid-cols-[1fr_1.2fr]">
        <Reveal>
        ...
        <div className="grid gap-4">
          <div className="flex flex-wrap gap-2">
            {links.map((p) => (
              <button key={p.id} type="button" onClick={() => goTo(p.id)} className="rounded-full border border-white/15 px-4 py-2 text-xs font-bold text-white/75">{tx(p.label)}</button>
            ))}
          </div>
...
export default function LobbyhausPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...lobbyhausDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="lobbyhaus" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: lobbyhausEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={lobbyhausPages.map((p) => ({
          id: p.id,
