...
const allowedPages = uxforgePages.map((p) => p.id);
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 rounded-b-[2rem] border-b border-white/15 bg-[var(--dark)]/90 text-white backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-full bg-[var(--p)] text-sm font-black text-white">UX</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "uxforge:PageHero:9:wireframe craft";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "uxforge:Hero:9:wireframe craft";
  const stats = [
...
function About({ data }: { data: Record<string, any> }) {
  void "uxforge:About:9:wireframe craft";
  return (
        ...
        <Reveal variant="right" className="">
          <div className="relative overflow-hidden rounded-[3rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">WIREFRAME CRAFT</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "uxforge:Services:9:wireframe craft";
  const services = [
...
function Cases({ data }: { data: Record<string, any> }) {
  void "uxforge:Cases:9:wireframe craft";
  const items = [
...
function Team({ data }: { data: Record<string, any> }) {
  void "uxforge:Team:9:wireframe craft";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "uxforge:Gallery:9:wireframe craft";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "uxforge:WhyUs:9:wireframe craft";
  const items = [
...
function Method({ data }: { data: Record<string, any> }) {
  void "uxforge:Method:9:wireframe craft";
...
function Stats({ data }: { data: Record<string, any> }) {
  void "uxforge:Stats:9:wireframe craft";
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "uxforge:Outcomes:9:wireframe craft";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "uxforge:Insights:9:wireframe craft";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "uxforge:Pricing:9:wireframe craft";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "uxforge:Faq:9:wireframe craft";
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "uxforge:CTABand:9:wireframe craft";
  return (
...
function Contact({ data }: { data: Record<string, any> }) {
  void "uxforge:Contact:9:wireframe craft";
  return (
    <section className="bg-[var(--dark)] text-white px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[2rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="uxforge-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "uxforge:Footer:9:wireframe craft";
  const links = uxforgePages.filter((p) => p.id !== "home");
  return (
...
export default function UxforgePages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...uxforgeDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="uxforge" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: uxforgeEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={uxforgePages.map((p) => ({
          id: p.id,
