...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3"  data-bizuply-block="lead-form" data-bizuply-form-id="dwellist-contact-1" data-bizuply-crm-lead="true" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אליכם בהקדם." >
      <input name="name" data-bizuply-form-field-id="name" autoComplete="name"  className={field} style={{ borderColor: "rgba(44,36,25,0.12)", color: "#2c2419" }} placeholder="שם מלא" />
      <input name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel"  className={field} style={{ borderColor: "rgba(44,36,25,0.12)", color: "#2c2419" }} placeholder="טלפון" />
      <input name="email" data-bizuply-form-field-id="email" type="email" autoComplete="email"  className={field} style={{ borderColor: "rgba(44,36,25,0.12)", color: "#2c2419" }} placeholder="אימייל" />
      <textarea name="message" data-bizuply-form-field-id="message"  className={cx(field, "min-h-28")} style={{ borderColor: "rgba(44,36,25,0.12)", color: "#2c2419" }} placeholder="מה אתם מחפשים?" />
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 md:grid-cols-3 gap-4">
        {stats.map(([vk,lk],i) => (
          <div key={lk} className="tpl-climb border p-5 text-center" style={{ borderColor: "rgba(44,36,25,0.12)", animationDelay: `${i*0.1}s` }}>
            <div className="tpl-display text-2xl sm:text-4xl font-bold" style={{ color: "#5c7c6a" }}>{v(data,vk)}</div>
