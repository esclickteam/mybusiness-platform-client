...
export default function JewelisThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative flex h-full min-h-[260px] w-full flex-col justify-between overflow-hidden p-5 text-right" style={{ background: "#0C0A09", color: "#FAF7F0", fontFamily: "Heebo, sans-serif" }}>
      ...
      <div className="relative">
        <div className="inline-flex px-2 py-1 text-[10px] font-black uppercase tracking-[0.2em]" style={{ background: "#A16207", color: "#1C1408" }}>{tx("תכשיטים ושעונים")}</div>
        <h3 className="mt-4 text-3xl font-black leading-none" style={{ fontFamily: "Cormorant, serif" }}>Jewelis</h3>
        <p className="mt-2 text-xs font-semibold opacity-70">{tx("יוקרה עדינה ליום־יום")}</p>
      </div>
