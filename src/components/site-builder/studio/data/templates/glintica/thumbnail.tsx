...
export default function GlinticaThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-5" style={{ background: "#1F1A1C", color: "#F8F1F2" }}>
      ...
      <div className="relative z-10 flex items-center justify-between border-b pb-3" style={{ borderColor: "#ffffff18" }}>
        ...
        <span className="text-[9px] tracking-[0.26em]" style={{ color: "#D4A0A7" }}>{tx("איפור כלות")}</span>
      </div>
      <div className="relative z-10 mt-7 text-center">
        ...
        <p className="mx-auto mt-4 max-w-[210px] text-xl font-semibold leading-none" style={{ fontFamily: "\"Cormorant Infant\", serif" }}>{tx("יופי נקי. נוכחות בלתי נשכחת.")}</p>
      </div>
      <div className="relative z-10 mt-7 grid grid-cols-[1fr_44px] gap-3">
        <div className="space-y-2">
          {[tx("איפור כלה"), tx("איפור ערב"), tx("ליווי")].map((item, index) => (
