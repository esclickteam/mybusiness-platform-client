...
export const nestoraPages = [
  ...
  { id: "find", label: "חיפוש", slug: "/find" },
  { id: "sell", label: "מכירה", slug: "/sell" },
...
function Header({ data, currentPage, goTo, onCta }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void; onCta: () => void }) {
  ...
  return (
    <header data-template-section-type="header" data-section-kind="header" className="sticky top-0 z-50 border-b"
      style={{ background: "#f3f5f8f2", borderColor: "rgba(30,40,55,0.14)", backdropFilter: "blur(12px)" }}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="flex items-center gap-3 text-right">
          <span className="grid h-10 w-10 place-items-center text-sm font-bold" style={{ background: "#3d5a80", color: "#0F141C" }}>{v(data, "logoText")}</span>
          <span className="tpl-display text-xl font-bold tracking-tight">{v(data, "brandName")}</span>
        </button>
        <nav className="hidden items-center gap-6 lg:flex">
          {nav.map(([id, label]) => (
            <button key={id} type="button" onClick={() => goTo(id)} className="text-sm font-semibold"
              style={{ color: currentPage === id ? "#1e2836" : "#667788" }}>{label}</button>
          ))}
        </nav>
        <div className="flex items-center gap-2">
          <button type="button" onClick={onCta} className="hidden px-5 py-2.5 text-sm font-bold sm:inline-flex"
            style={{ background: "#3d5a80", color: "#f3f5f8" }}>{v(data, "heroPrimary")}</button>
          <button type="button" onClick={() => setOpen((x) => !x)} className="grid h-10 w-10 place-items-center border lg:hidden" style={{ borderColor: "rgba(30,40,55,0.14)" }}>{open ? "×" : "☰"}</button>
        </div>
      </div>
      {open ? (
        <div className="border-t px-5 pb-4 lg:hidden" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" onSubmit={(e) => e.preventDefault()}>
      <input className={field} style={{ borderColor: "rgba(30,40,55,0.14)", color: "#1e2836" }} placeholder="שם מלא" />
      <input className={field} style={{ borderColor: "rgba(30,40,55,0.14)", color: "#1e2836" }} placeholder="טלפון" />
      <input className={field} style={{ borderColor: "rgba(30,40,55,0.14)", color: "#1e2836" }} placeholder="אימייל" />
      <textarea className={cx(field, "min-h-28")} style={{ borderColor: "rgba(30,40,55,0.14)", color: "#1e2836" }} placeholder="מה אתם מחפשים?" />
      <button type="button" onClick={onCta} className="px-6 py-4 text-sm font-bold" style={{ background: "#3d5a80", color: "#f3f5f8" }}>{v(data, "cta")}</button>
    </form>
  );
}
...
function Hero({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  ...
  const actions = (
    <div className="mt-8 flex flex-wrap gap-3">
      <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#3d5a80", color: "#f3f5f8" }}>{v(data, "heroPrimary")}</button>
      <button type="button" onClick={() => goTo("find")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(30,40,55,0.14)" }}>{v(data, "heroSecondary")}</button>
    </div>
  );
  ...
  if (layout === "type") {
    return (
      <section className="relative isolate min-h-[88vh] overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(100deg, #f3f5f8f5 0%, #f3f5f890 50%, transparent 100%)" }} />
        <div className="relative z-10 mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-5 pb-16 pt-28 lg:px-8">
          <p className="text-xs font-semibold tracking-[0.26em]" style={{ color: "#3d5a80" }}>{v(data, "heroEyebrow")}</p>
          <h1 className="tpl-display mt-4 max-w-4xl text-6xl font-extrabold leading-[0.95] md:text-8xl">{v(data, "heroTitle")}</h1>
          <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#667788" }}>{v(data, "heroSubtitle")}</p>
          {actions}
        </div>
      </section>
    );
  ...
  if (layout === "poster") {
    return (
      <section className="mx-auto grid max-w-7xl lg:grid-cols-[0.95fr_1.05fr]">
        <div className="flex min-h-[70vh] flex-col justify-between border p-8 lg:p-12" style={{ borderColor: "rgba(30,40,55,0.14)", background: "#e6ebf2" }}>
          <div>
            <p className="text-xs font-semibold tracking-[0.26em]" style={{ color: "#3d5a80" }}>{v(data, "heroEyebrow")}</p>
            <h1 className="tpl-display mt-8 text-6xl font-bold leading-[0.95] md:text-7xl">{v(data, "heroTitle")}</h1>
          </div>
          <div>
            <p className="max-w-md text-lg leading-8" style={{ color: "#667788" }}>{v(data, "heroSubtitle")}</p>
            {actions}
          </div>
        </div>
        <div className="min-h-[50vh] border-t lg:border-t-0 lg:border-r" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
    ...
    return (
      <section className="mx-auto max-w-7xl px-5 py-14 lg:px-8 lg:py-20">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div>
            <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "#3d5a80" }}>{v(data, "heroEyebrow")}</p>
            <h1 className="tpl-display mt-4 text-5xl font-bold leading-[1.02] md:text-7xl">{v(data, "heroTitle")}</h1>
            <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#667788" }}>{v(data, "heroSubtitle")}</p>
            {actions}
          </div>
          <div className="min-h-[340px] border" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
  ...
  return (
    <section className="relative isolate min-h-[86vh] overflow-hidden">
      <img src={v(data, "heroImage")} alt="" className="absolute inset-0 h-full w-full object-cover" />
      <div className="absolute inset-0" style={{ background: "linear-gradient(105deg, #f3f5f8f2 0%, #f3f5f895 46%, transparent 78%)" }} />
      <div className="relative z-10 mx-auto flex min-h-[86vh] max-w-7xl items-end px-5 pb-16 pt-28 lg:px-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-6 max-w-lg text-lg leading-8" style={{ color: "#667788" }}>{v(data, "heroSubtitle")}</p>
          {actions}
        </div>
      </div>
    </section>
...
function ImageBand({ title, text, image, reverse = false }: { title: string; text: string; image: string; reverse?: boolean }) {
  return (
    <section className="border-t" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
      <div className={cx("mx-auto grid max-w-7xl lg:grid-cols-2", reverse && "lg:[&>*:first-child]:order-2")}>
        <div className="flex flex-col justify-center px-5 py-16 lg:px-12 lg:py-24">
          ...
          <p className="mt-6 max-w-md text-base leading-8" style={{ color: "#667788" }}>{text}</p>
        </div>
...
function ItemsList({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-20 lg:px-8 lg:py-28" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
      <div className="mx-auto max-w-7xl">
        <h2 className="tpl-display text-4xl font-bold md:text-5xl">{v(data, "roomsTitle")}</h2>
        <div className="mt-12">
          {items.map(([title, meta, text]) => (
            <div key={title} className="grid gap-3 border-t py-8 md:grid-cols-[1fr_0.55fr_1.2fr] md:items-baseline" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
              <h3 className="tpl-display text-2xl font-bold">{title}</h3>
              <p className="text-sm font-semibold" style={{ color: "#3d5a80" }}>{meta}</p>
              <p className="text-base leading-7" style={{ color: "#667788" }}>{text}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="border-t" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-5 py-16 lg:px-12 lg:py-24">
          ...
          <p className="mt-6 max-w-xl text-lg leading-8" style={{ color: "#667788" }}>{v(data, "aboutText")}</p>
        </div>
    ...
    <section className="border-t px-5 py-20 lg:px-8 lg:py-28" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
        <div>
          ...
          <p className="mt-5 max-w-md text-base leading-8" style={{ color: "#667788" }}>{v(data, "contactText")}</p>
          <div className="mt-10 space-y-3 text-sm">
            <p><span style={{ color: "#667788" }}>טלפון · </span>{v(data, "phone")}</p>
            <p><span style={{ color: "#667788" }}>אימייל · </span>{v(data, "email")}</p>
            <p><span style={{ color: "#667788" }}>כתובת · </span>{v(data, "address")}</p>
          </div>
        </div>
...
function Footer({ data }: { data: Record<string, any> }) {
  return (
    <footer className="border-t px-5 py-8 lg:px-8" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
      <div className="mx-auto flex max-w-7xl flex-col gap-3 text-sm md:flex-row md:items-center md:justify-between" style={{ color: "#667788" }}>
        <p>© {new Date().getFullYear()} {v(data, "brandName")}</p>
        <p>תבנית Nestora · Bizuply Studio</p>
      </div>
    </footer>
  );
}
...
function InnerPage({ data, title, children, onCta }: { data: Record<string, any>; title: string; children: React.ReactNode; onCta: () => void }) {
  return (
    <>
      <section className="border-b px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(30,40,55,0.14)" }}>
        <div className="mx-auto max-w-7xl">
...
export default function NestoraPages({
  ...
  return (
    <div dir="rtl" data-template-id={mode === "preview" ? "nestora-preview" : "nestora"} className="min-h-screen w-full overflow-x-hidden"
      style={{ background: "#f3f5f8", color: "#1e2836" }}>
