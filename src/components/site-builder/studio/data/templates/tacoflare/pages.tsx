...
export const tacoflarePages = [
  { id: "home", label: "בית", slug: "/" },
  { id: "tacos", label: "טאקוס", slug: "/tacos" },
  { id: "salsa", label: "סלסות", slug: "/salsa" },
  { id: "about", label: "אודות", slug: "/about" },
  { id: "contact", label: "הזמנה", slug: "/contact" },
];

const allowedPages = tacoflarePages.map((p) => p.id);
...
function v(data: Record<string, any>, key: string) {
  return data?.[key] ?? (tacoflareDefaultData as Record<string, any>)[key] ?? "";
}
  ...
  return (
    <header data-template-section-type="header" data-section-kind="header" className="sticky top-0 z-50" style={{ background: "transparent" }}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="-rotate-2 border-2 px-3 py-2" style={{ borderColor: "#e85d04", background: "#251610" }}>
        ...
        <nav className="hidden items-center gap-5 lg:flex">
          {nav.map(([id, label]) => (
            <button key={id} type="button" onClick={() => goTo(id)} className="text-sm font-semibold"
              style={{ color: currentPage === id ? "#fff3e8" : "#c49a7a" }}>{label}</button>
          ))}
        </nav>
        <button type="button" onClick={onCta} className="border-2 px-4 py-2 text-sm font-black" style={{ borderColor: "#e85d04", color: "#e85d04" }}>{v(data, "heroPrimary")}</button>
      </div>
    </header>
  );
}
...
function Hero({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
      <section className="relative isolate min-h-[92vh] overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, #0d070588, #1a0e0af2)" }} />
        <div className="relative z-10 mx-auto flex min-h-[92vh] max-w-7xl flex-col justify-end px-5 pb-16 pt-28 lg:px-8">
          ...
          <p className="tpl-rise-3 mt-6 max-w-xl text-lg leading-8" style={{ color: "#c49a7a" }}>{v(data, "heroSubtitle")}</p>
          <div className="tpl-rise-3 mt-8 flex flex-wrap gap-3">
            <button type="button" onClick={onCta} className="px-7 py-3.5 text-sm font-bold" style={{ background: "#e85d04", color: "#1a0e0a" }}>{v(data, "heroPrimary")}</button>
            <button type="button" onClick={() => goTo("tacos")} className="border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "rgba(255,243,232,0.12)" }}>{v(data, "heroSecondary")}</button>
          </div>
        </div>
      </section>
  );
...
function PapelFlutterCards({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8 lg:py-20" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-6">
          {cards.map((c, i) => (
            <Reveal key={c.title} delayMs={i * 100} variant="scale">
              <article className={"tpl-papel w-64 border-2 p-3 " + (i === 1 ? "rotate-2" : i === 0 ? "-rotate-3" : "rotate-1")} style={{ borderColor: "#e85d04", background: "#251610", animationDelay: `${i * 0.2}s` }}>
                <img src={c.img} alt="" className="aspect-square w-full object-cover" />
                <p className="mt-3 text-xs font-bold" style={{ color: "#e85d04" }}>{c.meta}</p>
                <h3 className="tpl-display mt-1 text-xl font-bold">{c.title}</h3>
                <p className="mt-2 text-sm" style={{ color: "#c49a7a" }}>{c.text}</p>
              </article>
            </Reveal>
          ))}
        </div>
...
function PapelProcessSteps({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
...
function PapelHomeGallery({ data }: { data: Record<string, any> }) {
  const imgs = [v(data, "galleryImage1"), v(data, "galleryImage2"), v(data, "galleryImage3"), v(data, "galleryImage4")];
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto max-w-7xl">
...
function PapelHomeReviews({ data }: { data: Record<string, any> }) {
  const revs = [1, 2, 3].map((i) => [v(data, `review${i}Text`), v(data, `review${i}Name`), v(data, `review${i}Role`)]);
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
...
function PapelHomeStats({ data }: { data: Record<string, any> }) {
  const stats = [[v(data, "stat1"), v(data, "stat1Label")], [v(data, "stat2"), v(data, "stat2Label")], [v(data, "stat3"), v(data, "stat3Label")]];
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-4">
        ...
        <div className="border p-4 md:col-span-1" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
          <p className="text-xs font-bold tracking-wider" style={{ color: "#e85d04" }}>{v(data, "hoursTitle")}</p>
          <p className="mt-2 text-sm leading-7" style={{ color: "#c49a7a" }}>{v(data, "hours")}</p>
        </div>
      </div>
    </section>
  );
...
function PapelHomeCtaTeaser({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 border p-8 md:flex-row md:items-center" style={{ borderColor: "#e85d04", background: "#251610" }}>
        <div>
...
function PapelSpecialtyBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-b px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2 lg:items-end">
        <div>
          ...
          <p className="mt-4 text-lg" style={{ color: "#c49a7a" }}>{v(data, "page1Subtitle")}</p>
        </div>
...
function PapelCategoryGrid({ data }: { data: Record<string, any> }) {
      ...
      <div className="mx-auto grid max-w-7xl gap-3 md:grid-cols-4">
        {cats.map(([t, x], i) => (
          <Reveal key={t} delayMs={i * 70} variant="up">
            <div className="border p-5" style={{ borderColor: "rgba(255,243,232,0.12)", background: i % 2 ? "#251610" : "#1a0e0a" }}>
              <h3 className="tpl-display text-xl font-bold" style={{ color: "#e85d04" }}>{t}</h3>
              <p className="mt-2 text-sm" style={{ color: "#c49a7a" }}>{x}</p>
            </div>
          </Reveal>
        ))}
      </div>
...
function PapelFullMenuBoard({ data }: { data: Record<string, any> }) {
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map(([title, meta, text, img], i) => (
            <Reveal key={title} delayMs={i * 50} variant="up">
              <article className="overflow-hidden border" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
...
function PapelChefPicks({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
          ...
          <p className="mt-4 text-lg leading-8" style={{ color: "#c49a7a" }}>{v(data, "chefPickText")}</p>
        </div>
...
function PapelPairingNotes({ data }: { data: Record<string, any> }) {
          ...
          {pairs.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 80} variant="up">
              <div className="border-r pr-4" style={{ borderColor: "#e85d04" }}>
                ...
                <p className="mt-2 text-sm leading-7" style={{ color: "#c49a7a" }}>{x}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
function PapelStoryBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="relative overflow-hidden px-5 py-20 lg:px-8" style={{ background: "#251610" }}>
      <div className="mx-auto max-w-7xl">
        <p className="text-xs tracking-[0.28em]" style={{ color: "#e85d04" }}>{v(data, "brandName")}</p>
        <h1 className="tpl-display mt-4 max-w-3xl text-5xl font-bold md:text-6xl">{v(data, "page2Title")}</h1>
        <p className="mt-4 max-w-xl text-lg" style={{ color: "#c49a7a" }}>{v(data, "page2Subtitle")}</p>
      </div>
    </section>
  );
}
function PapelTechniqueLadder({ data }: { data: Record<string, any> }) {
        ...
        <div className="mt-10 space-y-6">
          {steps.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 70} variant="right">
              <div className="grid gap-2 border p-5 md:grid-cols-[60px_1fr]" style={{ borderColor: "rgba(255,243,232,0.12)", background: i % 2 ? "#251610" : "#1a0e0a" }}>
                <span className="tpl-display text-3xl font-bold" style={{ color: "#e85d04" }}>0{i + 1}</span>
                <div>
                  <h3 className="tpl-display text-xl font-bold">{t}</h3>
                  <p className="mt-2 text-sm leading-7" style={{ color: "#c49a7a" }}>{x}</p>
                </div>
...
function PapelMaterialCards({ data }: { data: Record<string, any> }) {
    ...
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {mats.map(([t, x], i) => (
            <Reveal key={t} delayMs={i * 70} variant="up">
              <div className="border p-6" style={{ borderColor: "#e85d04" }}>
                <h3 className="tpl-display text-xl font-bold">{t}</h3>
                <p className="mt-3 text-sm" style={{ color: "#c49a7a" }}>{x}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
function PapelEventsBand({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2 lg:items-center">
        ...
        <div>
          ...
          <p className="mt-4 text-lg leading-8" style={{ color: "#c49a7a" }}>{v(data, "eventsText")}</p>
...
function PapelAboutBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-b px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto max-w-7xl">
        ...
        <p className="mt-4 max-w-2xl text-lg leading-8" style={{ color: "#c49a7a" }}>{v(data, "aboutPageLead")}</p>
      </div>
    </section>
  );
}
function PapelAboutTimeline({ data }: { data: Record<string, any> }) {
    ...
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
      <div className="mx-auto max-w-3xl">
        ...
        <div className="relative mt-10">
          <div className="absolute right-3 top-0 bottom-0 w-px" style={{ background: "rgba(255,243,232,0.12)" }} />
...
function PapelChefPortrait({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2 lg:items-center">
        ...
        <div>
          ...
          <p className="mt-4 leading-8" style={{ color: "#c49a7a" }}>{v(data, "chefBio")}</p>
          <blockquote className="mt-8 border-r-4 pr-4 text-xl" style={{ borderColor: "#e85d04" }}>״{v(data, "chefQuote")}״</blockquote>
        </div>
      </div>
    </section>
  );
}
function PapelValuesRow({ data }: { data: Record<string, any> }) {
        ...
        {vals.map(([t, x], i) => (
          <Reveal key={t} delayMs={i * 70} variant="up">
            <div className="p-5">
              ...
              <p className="mt-3 text-sm leading-7" style={{ color: "#c49a7a" }}>{x}</p>
            </div>
          </Reveal>
        ))}
      </div>
...
function PapelContactBanner({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-b px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto max-w-7xl">
        ...
        <p className="mt-4 max-w-xl text-lg" style={{ color: "#c49a7a" }}>{v(data, "contactPageText")}</p>
      </div>
    </section>
  );
}
function PapelReserveForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto max-w-xl text-center">
        ...
        <form className="mt-8 grid gap-3 rounded-[2rem] border p-6 text-right" style={{ borderColor: "#e85d04", background: "#251610" }}  data-bizuply-block="lead-form" data-bizuply-form-id="tacoflare-contact-1" data-bizuply-crm-lead="true" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אליכם בהקדם." >
          <input name="name" data-bizuply-form-field-id="name" autoComplete="name"  className="w-full rounded-full border bg-transparent px-4 py-3.5 outline-none" style={{ borderColor: "rgba(255,243,232,0.12)", color: "#fff3e8" }} placeholder="שם מלא" />
          <input name="phone" data-bizuply-form-field-id="phone" type="tel" autoComplete="tel"  className="w-full rounded-full border bg-transparent px-4 py-3.5 outline-none" style={{ borderColor: "rgba(255,243,232,0.12)", color: "#fff3e8" }} placeholder="טלפון" />
...
function PapelHoursMap({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">
        <div className="border p-6" style={{ borderColor: "rgba(255,243,232,0.12)", background: "#251610" }}>
          <h3 className="tpl-display text-2xl font-bold">{v(data, "hoursTitle")}</h3>
          <p className="mt-4 text-sm leading-7" style={{ color: "#c49a7a" }}>{v(data, "hours")}</p>
          <p className="mt-4 text-sm">{v(data, "address")}</p>
        </div>
        <div className="relative min-h-[220px] overflow-hidden border" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
          <img src={v(data, "galleryImage1")} alt="" className="absolute inset-0 h-full w-full object-cover opacity-60" />
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="border px-4 py-2 text-xs font-bold tracking-wider" style={{ borderColor: "#e85d04", background: "#1a0e0a", color: "#e85d04" }}>{v(data, "mapLabel")}</span>
          </div>
        </div>
      </div>
    </section>
...
function PapelFaqBlock({ data }: { data: Record<string, any> }) {
      ...
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        {faqs.map(([q, a], i) => (
          <Reveal key={q} delayMs={i * 70} variant="up">
            <details className="border p-4" style={{ borderColor: "rgba(255,243,232,0.12)" }}>
              <summary className="cursor-pointer font-bold">{q}</summary>
              <p className="mt-3 text-sm leading-7" style={{ color: "#c49a7a" }}>{a}</p>
            </details>
          </Reveal>
        ))}
      </div>
...
function Footer({ data }: { data: Record<string, any> }) {
  return (
    <footer className="px-5 py-8 lg:px-8">
      <div className="mx-auto max-w-7xl border-2 border-dashed p-5" style={{ borderColor: "#e85d04" }}>
        <div className="flex flex-col gap-2 text-sm md:flex-row md:justify-between" style={{ color: "#c49a7a" }}>
          <span className="tpl-display text-lg font-black" style={{ color: "#fff3e8" }}>{v(data, "brandName")}</span>
...
function HomePage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      ...
      <PapelFlutterCards data={data} />
      <PapelProcessSteps data={data} />
      <PapelHomeGallery data={data} />
      <PapelHomeReviews data={data} />
      <PapelHomeStats data={data} />
      <PapelHomeCtaTeaser data={data} goTo={goTo} />
      <Footer data={data} />
    </>
  );
}

function TacosPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <PapelSpecialtyBanner data={data} />
      <PapelCategoryGrid data={data} />
      <PapelFullMenuBoard data={data} />
      <PapelChefPicks data={data} />
      <PapelPairingNotes data={data} />
      <Footer data={data} />
    </>
  );
}

function SalsaPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <PapelStoryBanner data={data} />
      <PapelTechniqueLadder data={data} />
      <PapelMaterialCards data={data} />
      <PapelEventsBand data={data} />
      <Footer data={data} />
    </>
  );
}
...
function AboutPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <PapelAboutBanner data={data} />
      <PapelAboutTimeline data={data} />
      <PapelChefPortrait data={data} />
      <PapelValuesRow data={data} />
      <Footer data={data} />
    </>
  );
}
...
function ContactPage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      <PapelContactBanner data={data} />
      <PapelReserveForm data={data} onCta={onCta} />
      <PapelHoursMap data={data} />
      <PapelFaqBlock data={data} />
      <Footer data={data} />
    </>
  );
}

export default function TacoflarePages({
  initialPage = "home", initialPageId, page, pageId, activePageId, currentPageId,
  mode = "preview", data, onPageChange, isPublic, viewMode, runtimeMode,
}: Props) {
  const merged = useMemo(() => ({ ...tacoflareDefaultData, ...(data ?? {}) }), [data]);
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    tacos: <TacosPage data={merged} goTo={goTo} onCta={() => goTo("contact")} />,
    salsa: <SalsaPage data={merged} goTo={goTo} onCta={() => goTo("contact")} />,
