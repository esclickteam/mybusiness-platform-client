...
const allowedPages = crisisdeskPages.map((p) => p.id);
...
function getValue(data: Record<string, any>, key: string) {
  return data?.[key] ?? (crisisdeskDefaultData as Record<string, any>)[key] ?? "";
}

function Header({ data, currentPage, goTo }: { data: Record<string, any>; currentPage: string; goTo: (id: string) => void }) {
  void "crisisdesk:Header:12:newsroom alert";
    ...
    <header data-visual-flow-lock="true" data-template-section-type="header" className="sticky top-0 z-50 rounded-b-[2rem] border-b border-white/15 bg-[var(--dark)]/90 text-white backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <button type="button" onClick={() => goTo("home")} className="group flex items-center gap-3 text-start">
          <span className="ag-pulse grid h-11 w-11 place-items-center rounded-2xl bg-[var(--p)] text-sm font-black text-white">CR</span>
          <span>
...
function PageHero({ data, pageId }: { data: Record<string, any>; pageId: string }) {
  void "crisisdesk:PageHero:12:newsroom alert";
...
function Hero({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "crisisdesk:Hero:12:newsroom alert";
  const stats = [
  ...
  return (
    <section data-template-section-type="hero" className="relative overflow-hidden bg-[var(--dark)] px-5 py-16 text-white lg:px-8">
      ...
      <div className="mx-auto grid min-h-[86svh] max-w-7xl items-center gap-8 lg:grid-cols-[0.92fr_1.08fr]">
        <Reveal variant="right" className="relative z-10 text-start">
...
function About({ data }: { data: Record<string, any> }) {
  void "crisisdesk:About:12:newsroom alert";
  return (
        ...
        <Reveal variant="scale" className="order-2 lg:order-1">
          <div className="relative overflow-hidden rounded-[3rem] border border-current/10 bg-[var(--surface)] p-3">
            ...
            <div className="absolute bottom-6 right-6 rounded-3xl bg-[var(--dark)]/80 p-5 text-white backdrop-blur">
              <p className="text-xs font-black text-[var(--a)]">NEWSROOM ALERT</p>
...
function Services({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "crisisdesk:Services:12:newsroom alert";
  const services = [
  ...
  return (
    <section className="bg-[var(--surface)] px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto max-w-7xl">
        <Reveal className="max-w-4xl text-start">
...
function Cases({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Cases:12:newsroom alert";
  const items = [
...
function Team({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Team:12:newsroom alert";
  const people = [
...
function Gallery({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Gallery:12:newsroom alert";
  const images = [
...
function WhyUs({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "crisisdesk:WhyUs:12:newsroom alert";
  const items = [
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {items.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 90} variant="up">
              <article className="ag-card rounded-[2rem] border border-current/10 bg-[var(--surface)] p-7 text-start">
...
function Method({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Method:12:newsroom alert";
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {steps.map(([title, text], i) => (
            <Reveal key={title} delayMs={i * 100} variant="up">
              <article className="ag-card rounded-[2rem] border border-current/10 bg-[var(--surface)]/80 p-6 text-start text-[var(--text)]">
...
function Stats({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Stats:12:newsroom alert";
      ...
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-4">
        {stats.map(([value, label], i) => (
          <Reveal key={label} delayMs={i * 80} variant="up">
            <div className="ag-card rounded-[2rem] border border-current/10 bg-[var(--surface)] p-7 text-start">
...
function Outcomes({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "crisisdesk:Outcomes:12:newsroom alert";
  const items = [
...
function Insights({ data, goTo }: { data: Record<string, any>; goTo?: (id: string) => void }) {
  void "crisisdesk:Insights:12:newsroom alert";
  const items = [
...
function Pricing({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Pricing:12:newsroom alert";
...
function Faq({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Faq:12:newsroom alert";
        ...
        <div className="mt-10 grid gap-3">
          {faqs.map(([q, a], i) => (
            <Reveal key={q} delayMs={i * 80} variant="up">
              <article className="ag-card rounded-[2rem] border border-current/10 bg-[var(--surface)] p-6 text-start">
...
function CTABand({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "crisisdesk:CTABand:12:newsroom alert";
  return (
    <section className="px-5 py-12 lg:px-8">
      <Reveal variant="scale" className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[var(--p)] p-8 text-start text-white md:p-12">
...
function Contact({ data }: { data: Record<string, any> }) {
  void "crisisdesk:Contact:12:newsroom alert";
  return (
    <section className="bg-[var(--surface)] px-5 py-20 lg:px-8 lg:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <Reveal variant="scale" className="rounded-[2rem] border border-current/10 bg-[var(--bg)]/60 p-5 md:p-8">
          <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-skin="template" data-bizuply-form-id="crisisdesk-contact" data-bizuply-success-message={tx("תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.")}>
...
function Footer({ data, goTo }: { data: Record<string, any>; goTo: (id: string) => void }) {
  void "crisisdesk:Footer:12:newsroom alert";
  const links = crisisdeskPages.filter((p) => p.id !== "home");
  return (
...
export default function CrisisdeskPages({
  initialPage = "home",
  initialPageId,
  page,
  pageId,
  ...
  const mergedData = useMemo(() => ({ ...crisisdeskDefaultData, ...(data ?? {}) }), [data]);
  ...
  return (
    <div dir={templateDir()} data-template-id="crisisdesk" className="min-h-screen w-full overflow-x-hidden">
      <style dangerouslySetInnerHTML={{ __html: crisisdeskEditorCss }} />
      <Header data={mergedData} currentPage={currentPage} goTo={goTo} />
      <VisualPageStack
        activePageId={currentPage}
        pages={crisisdeskPages.map((p) => ({
          id: p.id,
