...
export default function SteelworksThumbnail() {
  ...
  return (
    <div dir={getTextDirection(i18n.language)} className="relative h-full min-h-[260px] w-full overflow-hidden p-5" style={{ background: "#1a1a1a", color: "#f0f0f0" }}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "#ff6b2c" }}>
          {tx("ייצור תעשייתי")}</span>
      </div>
      <div className="mt-10">
        <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.24em]" style={{ color: "#ff6b2c" }}>
          {tx("ייצור תעשייתי")}</p>
        <h3 className="max-w-[220px] text-2xl font-black leading-tight">{tx("חוזק שאפשר למדוד.")}</h3>
