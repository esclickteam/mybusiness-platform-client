...
function ContactForm({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  ...
  return (
    <form className="grid gap-3" data-bizuply-block="lead-form" data-bizuply-crm-lead="true" data-bizuply-form-builder="true" data-bizuply-form-id="landmark-contact" data-bizuply-success-message="תודה! קיבלנו את הפנייה ונחזור אלייך בהקדם.">
...
function PinGalleryWall({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 md:grid-cols-4">
        {[1,2,3,4].map((i) => (
          <div key={i} className="relative">
            <div className="tpl-pin absolute -left-1 -top-1 z-10 h-4 w-4 rounded-full" style={{ background: "#0ea5e9" }} />
...
function PinAgentRoster({ data }: { data: Record<string, any> }) {
      ...
      <div className="mx-auto max-w-7xl divide-y" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
        {agents.map((a) => (
          <div key={a.n} className="flex flex-wrap items-center gap-5 py-6">
            ...
            <div><h3 className="tpl-display text-xl font-bold">{a.n}</h3><p style={{ color: "#0ea5e9" }}>{a.r} · {a.d}</p></div>
          </div>
        ))}
      </div>
    </section>
...
function PinQuoteRail({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-14 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
      <div className="mx-auto max-w-3xl border-r-4 pr-8" style={{ borderColor: "#0ea5e9" }}>
...
function PinTrustMetrics({ data }: { data: Record<string, any> }) {
  const stats = [["stat1Value","stat1Label"],["stat2Value","stat2Label"],["stat3Value","stat3Label"]];
  return (
    <section className="border-t px-5 py-12 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
      <div className="mx-auto grid max-w-7xl grid-cols-1 md:grid-cols-3 gap-4">
        {stats.map(([vk,lk],i) => (
          <div key={lk} className="tpl-climb border p-5 text-center" style={{ borderColor: "rgba(30,41,59,0.1)", animationDelay: `${i*0.1}s` }}>
            <div className="tpl-display text-2xl sm:text-4xl font-bold" style={{ color: "#0ea5e9" }}>{v(data,vk)}</div>
...
function PinMarketPulse({ data }: { data: Record<string, any> }) {
      ...
      <div className="mx-auto mt-10 grid max-w-7xl gap-5 lg:grid-cols-3">
        {posts.map(([t,x,g]) => (
          <article key={t} className="border p-5" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
            <span className="text-[10px] font-bold" style={{ color: "#0ea5e9" }}>{g}</span>
...
function PinCtaRibbon({ data, onCta }: { data: Record<string, any>; onCta: () => void }) {
  return (
    <section className="px-5 py-14 lg:px-8" style={{ background: "#0ea5e9" }}>
...
function PinFaqPanel({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
      <div className="mx-auto max-w-3xl grid gap-2">
        {faqs.map(([q,a]) => (
          <details key={q} className="border" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
            <summary className="cursor-pointer px-4 py-3 font-bold">{q}</summary>
            <p className="border-t px-4 py-3 text-sm leading-7" style={{ borderColor: "rgba(30,41,59,0.1)", color: "#64748b" }}>{a}</p>
          </details>
        ))}
      </div>
    </section>
...
function PinOfficeBlock({ data }: { data: Record<string, any> }) {
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2">
        ...
        <div className="relative min-h-[280px] overflow-hidden border" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
...
function PinAwardsLane({ data }: { data: Record<string, any> }) {
  const awards = [v(data,"award1"),v(data,"award2"),v(data,"award3"),v(data,"award4")];
  return (
    <section className="overflow-hidden border-y py-4" style={{ borderColor: "rgba(30,41,59,0.1)", background: "#f0f4f8" }}>
      <div className="tpl-marquee-track gap-10 px-6 text-xs font-bold tracking-[0.25em]" style={{ color: "#0ea5e9" }}>
...
function PinProcessRail({ data }: { data: Record<string, any> }) {
...
function PinListingGrid({ data }: { data: Record<string, any> }) {
  ...
  return (
    <section className="border-t px-5 py-16 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)", background: "#ffffff" }}>
      <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-2 lg:grid-cols-4">
        {cards.map((c) => (
          <article key={c.t} className="overflow-hidden border" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
            <img src={c.img} alt="" className="aspect-[4/3] w-full object-cover" />
            <div className="p-4"><p className="text-xs" style={{ color: "#0ea5e9" }}>{c.m}</p><h3 className="font-bold">{c.t}</h3><p className="mt-2 font-bold" style={{ color: "#0ea5e9" }}>{c.p}</p></div>
          </article>
        ))}
      </div>
    </section>
...
function PinStatsRow({ data }: { data: Record<string, any> }) {
  const stats = [["stat1Value","stat1Label"],["stat2Value","stat2Label"],["stat3Value","stat3Label"]];
  return (
    <section className="border-t px-5 py-10 lg:px-8" style={{ borderColor: "rgba(30,41,59,0.1)" }}>
...
function HomePage({ data, goTo, onCta }: { data: Record<string, any>; goTo: (id: string) => void; onCta: () => void }) {
  return (
    <>
      ...
      <PinGalleryWall data={data} />
      <PinAwardsLane data={data} />
      <PinQuoteRail data={data} />
      <PinCtaRibbon data={data} onCta={onCta} />
  ...
  const pageContent: Record<string, React.ReactNode> = {
    ...
    contact: (
      <>
        ...
        <PinOfficeBlock data={merged} />
      <PinAgentRoster data={merged} />
      <PinFaqPanel data={merged} />
      <PinTrustMetrics data={merged} />
      <PinAwardsLane data={merged} />
        <Footer data={merged} />
      </>
    ),
  };
    pageContent["areas"] = (
      ...
      <PinMarketPulse data={merged} />
      <PinStatsRow data={merged} />
      <PinCtaRibbon data={merged} onCta={() => goTo("contact")} />
        </>
      </InnerPage>
    );
