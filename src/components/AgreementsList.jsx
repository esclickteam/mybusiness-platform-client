...
export default function AgreementsList() {
  ...
  const openAgreement = async (agreementId) => {
    try {
      // Convert to valid string before using in URL
      ...
      {modalOpen && selectedAgreement && (
        ...
        >
          ...
          >
            ...
            <p>
              <strong>Sender business:</strong> {selectedAgreement.sender?.businessName || "-"}
            </p>
            <p>
              <strong>Receiver business:</strong> {selectedAgreement.receiver?.businessName || "-"}
            </p>
            <p>
              <strong>Starts on:</strong>{" "}
              {selectedAgreement.startDate
                ? new Date(selectedAgreement.startDate).toLocaleDateString("en-US")
                : "-"}
            </p>
            <p>
              <strong>Ends on:</strong>{" "}
              {selectedAgreement.endDate
                ? new Date(selectedAgreement.endDate).toLocaleDateString("en-US")
                : "-"}
            </p>
              ...
              {selectedAgreement.signatures?.createdBy?.signed ? (
                <>
                  ...
                  {selectedAgreement.signatures.createdBy.signedAt
                    ? new Date(selectedAgreement.signatures.createdBy.signedAt).toLocaleDateString("en-US")
                    : "-"}
                </>
              ) : (
                "❌ Not signed"
              ...
              {selectedAgreement.signatures?.invitedBusiness?.signed ? (
                <>
                  ...
                  {selectedAgreement.signatures.invitedBusiness.signedAt
                    ? new Date(selectedAgreement.signatures.invitedBusiness.signedAt).toLocaleDateString("en-US")
                    : "-"}
                </>
              ) : (
                "❌ Not signed"
