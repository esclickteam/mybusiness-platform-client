    ...
    <section className="relative overflow-hidden bg-white py-16 text-slate-800 sm:py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-7 text-4xl font-black leading-[1.02] tracking-[-0.04em] text-slate-800 sm:text-5xl">
            {t("modules.titleTop")}{" "}
        ...
        <div className="mt-10 grid gap-4 sm:mt-14 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
          {MODULES.map(({ id, to, Icon, tone }) => (
            <Link
              key={id}
              to={to}
              className="group relative flex flex-col overflow-hidden rounded-[1.5rem] border border-slate-100 bg-white p-5 text-start shadow-[0_14px_40px_rgba(15,23,42,0.06)] transition duration-300 hover:-translate-y-1.5 hover:border-indigo-100 hover:shadow-[0_24px_70px_rgba(79,70,229,0.16)] sm:rounded-[1.75rem] sm:p-7"
            >
              <span
                className={`mb-4 grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br ${tone} text-white shadow-lg shadow-indigo-100 sm:mb-5 sm:h-12 sm:w-12`}
              >
              ...
              <span className="mt-4 inline-flex items-center gap-2 text-sm font-black text-indigo-700 sm:mt-6">
                {t("modules.open")}
                <span
                  aria-hidden="true"
