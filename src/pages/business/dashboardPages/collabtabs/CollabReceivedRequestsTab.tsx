...
type CollabReceivedRequestsTabProps = {
  isDevUser?: boolean;
...
export default function CollabReceivedRequestsTab({
  ...
  const acceptedCount = useMemo(() => {
    return receivedRequests.filter((request) => request.status === "accepted")
      .length;
  }, [receivedRequests]);

  const rejectedCount = useMemo(() => {
    return receivedRequests.filter((request) => request.status === "rejected")
      .length;
  }, [receivedRequests]);
  ...
  const handleAccept = async (proposalId: string) => {
    ...
    try {
      ...
      setReceivedRequests((prev) =>
        prev.map((proposal) =>
          getProposalId(proposal) === proposalId
            ? {
                ...proposal,
                status: "accepted",
                agreementId: agreementId || proposal.agreementId,
              }
            : proposal
        )
      );
  ...
  if (error) {
    return <ErrorState text={error} onRetry={fetchReceivedRequests} />;
  }
            ...
            {receivedRequests.map((request) => {
              ...
              return (
                <ReceivedRequestCard
                  key={proposalId}
                  request={request}
                  onAccept={() => handleAccept(proposalId)}
                  onReject={() => handleReject(proposalId)}
                />
              );
            })}
          </div>
...
function ReceivedRequestCard({
  ...
  return (
    <article className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-violet-100 hover:shadow-[0_20px_70px_rgba(15,23,42,0.10)]">
      ...
      {status === "accepted" && request.agreementId && (
        <div className="mt-5">
          <button
            type="button"
...
function getProposalId(proposal: ReceivedProposal) {
